...
impl Val {
    ...
    fn with_origins(state: St, origins: BTreeSet<String>) -> Val {
        Val { state, origins, temp_id: None, srcs: BTreeSet::new() }
    }

    fn borrowed_from(name: &str) -> Val {
        Val::with_origins(Borrowed, BTreeSet::from([name.to_string()]))
    }
...
fn env_key(env: &Env) -> Vec<(String, St, Vec<String>)> {
    let mut v: Vec<(String, St, Vec<String>)> = env
        .iter()
...
struct AssertCtx {
    asserts: Vec<(AssertMode, Vec<String>)>,
    frozen_here: Vec<String>,
    snapshots: HashMap<String, Option<Val>>,
}
...
struct Chk<'a> {
    ...
    next_temp: u64,
    frozen: HashSet<String>, // vars under an ASSERT block
    assert_depth: u32,       // nesting depth of ASSERT blocks
    reported: HashSet<(u32, &'static str, String)>, // dedup (line, cat, var)
    param_modes: IndexMap<String, Mode>,
    noun_params: Vec<String>,
    goto_envs: IndexMap<String, Vec<Env>>, // label -> envs parked by forward gotos
    label_pos: HashMap<String, u32>,       // label name -> source offset
}
...
impl<'a> Chk<'a> {
        ...
        match self.exec_stmt(&body, vec![env]) {
            Ok(flow) => {
                ...
                if !self.goto_envs.is_empty() {
                    ...
                    let mut labels: Vec<String> = self.goto_envs.keys().cloned().collect();
                    labels.sort();
                    let fun = self.fun;
                    self.report(
                        Some(&fun),
    ...
    fn check_exit(
        ...
        for p in self.noun_params.clone() {
            ...
            if mode == Mode::Retain {
                ...
                if returned_root == Some(p.as_str()) {
                    continue;
                }
                if v.state == Owned {
                    self.report(
        ...
        for (name, v) in env {
            ...
            if returned_root == Some(name.as_str()) {
                continue;
            }
            if v.state == Owned {
                self.report(
    ...
    fn consume(&mut self, cur: &Cursor, env: &mut Env, val: &Val, what: Option<String>) {
        ...
        let disp = || name.clone().unwrap_or_else(|| "?".to_string());
        match val.state {
            ...
            Borrowed => {
                ...
                if let Some(n) = &name {
                    env.insert(n.clone(), Val::new(Consumed));
                    for root in val.origins.iter().cloned().collect::<Vec<_>>() {
    ...
    fn bind_init_list(&mut self, d: &Cursor, init: &Cursor, env: &mut Env) -> R<()> {
        ...
        for (f, e) in fields.iter().zip(elems.iter()) {
            ...
            let key = format!("{}.{}", d.spelling(), f.spelling());
    ...
    fn eval_call(&mut self, cur: &Cursor, env: &mut Env) -> R<Val> {
        ...
        let cname: Option<String> = callee.as_ref().map(|c| c.spelling());
        ...
        for (a, p, v) in &evald {
            ...
            if let (Some(sem), Some(p)) = (&semo, p) {
                if sem.passthrough.as_deref() == Some(p.spelling().as_str()) {
            ...
            let pname = p.map(|p| p.spelling()).unwrap_or_default();
    ...
    fn bound_fact(&self, op: i32, a: &Cursor, b: &Cursor, env: &Env) -> Vec<(String, bool)> {
    ...
    fn guard_fact(
        ...
        b: &Cursor,
        env: &Env,
    ) -> Option<(String, Option<Val>, Option<Val>)> {
        for (x, y) in [(a, b), (b, a)] {
    ...
    fn assert_enter(&mut self, compound: &Cursor, env: &Env) -> AssertCtx {
        let asserts = self.host.block_asserts(compound);
    ...
    fn check_return_val(&mut self, cur: &Cursor, v: &Val, root: Option<&str>) {
        if let Some(pt) = &self.sem.passthrough {
            if root == Some(pt.as_str()) || v.srcs.contains(pt) {
