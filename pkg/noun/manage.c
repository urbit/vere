...
#       define u3m_trap() (u3_noun)(_setjmp(u3R->esc.buf))
      ...
      */
        u3_noun
        u3m_soft_top(c3_n    mil_w,                     //  timer ms
                     c3_n    pad_w,                     //  base memory pad
                     u3_funk fun_f,
                     u3_noun   arg);
...
*/
static u3_road*
_pave_road(c3_n* rut_w, c3_n* mat_w, c3_n* cap_w, c3_n siz_w)
{
  ...
  memset(rod_u, 0, sizeof(c3_n) * siz_w);

  //  the top and bottom of the heap are initially the same
  //
  rod_u->rut_p = u3of(c3_n, rut_w);
  rod_u->hat_p = u3of(c3_n, rut_w);


  rod_u->mat_p = u3of(c3_n, mat_w);  //  stack bottom
  rod_u->cap_p = u3of(c3_n, cap_w);  //  stack top
...
*/
static u3_road*
_pave_north(c3_n* mem_w, c3_n siz_w, c3_n len_w, c3_o kid_o)
{
  ...
  //
  c3_n* mat_w = c3_align(mem_w + len_w - siz_w, u3a_balign, C3_ALGLO);
  c3_n* rut_w = c3_align(mem_w, u3a_balign, C3_ALGHI);
  c3_n* cap_w = mat_w;

  if ( c3y == kid_o ) {
    u3e_ward(u3of(c3_n, rut_w) - 1, u3of(c3_n, cap_w));
  }
...
*/
static u3_road*
_pave_south(c3_n* mem_w, c3_n siz_w, c3_n len_w)
{
  ...
  //
  c3_n* mat_w = c3_align(mem_w, u3a_balign, C3_ALGHI);
  c3_n* rut_w = c3_align(mem_w + len_w, u3a_balign, C3_ALGLO);
  c3_n* cap_w = mat_w + siz_w;

  u3e_ward(u3of(c3_n, cap_w) - 1, u3of(c3_n, rut_w));
...
_pave_home(void)
{
  c3_n* mem_w = u3_Loom + u3a_walign;
  c3_n  siz_w = c3_wiseof(u3v_home);
  c3_n  len_w = u3C.wor_i - u3a_walign;
...
_find_home(void)
{
  c3_n ver_w = *(u3_Loom + u3C.wor_i - 1);
...
u3m_dump(void)
{
  ...
  if ( 0 != (hat_w - fre_w) ) {
    c3_n* box_w = _(u3a_is_north(u3R)) ? u3R->rut_w : u3R->hat_w;
    c3_n  mem_w = 0;
...
u3m_leap(c3_n pad_w)
{
  c3_n     len_w; /* the length of the new road (avail - (pad + wiseof(u3a_road))) */
  u3_road* rod_u;
  ...
  */
  {
    u3p(c3_n) bot_p;            /* S: bot_p = new mat. N: bot_p = new rut  */
...
*/
static c3_o
_cm_is_tas(u3_atom som, c3_n len_w)
{
  c3_n i_w;
...
*/
static c3_o
_cm_is_ta(u3_noun som, c3_n len_w)
{
  c3_n i_w;
...
{
  c3_n len_w = _cm_in_pretty(som, c3y, 0);
...
#if 1  // XX redundant
  {
    c3_n low_w = u3a_heap(u3R);  // old u3m_water()
    c3_n hig_w = u3a_temp(u3R) + c3_wiseof(u3v_home);

    c3_n nox_w = (low_w + ((1 << u3a_page) - 1)) >> u3a_page;
    c3_n sox_w = (hig_w + ((1 << u3a_page) - 1)) >> u3a_page;

    c3_n nor_w = (low_p + ((1 << u3a_page) - 1)) >> u3a_page;
...
u3m_init(size_t len_i)
{
  ...
  if (  !len_i
     || (len_i & (len_i - 1))
     || (len_i < (1 << (u3a_page + u3a_note_bytes_shift)))
     || (len_i > u3a_bytes) )
  {
  ...
  //
  {
    ...
    u3C.wor_i = len_i >> u3a_note_bytes_shift;
