...
_find_home(void)
{
  ...
  //  ;;: what about this looks risky? It looks normal to me. -- also, is this
  //  necessary? Shouldn't these values have already been read in or is it
...
/* ;;: consider moving all the migration code to a separate CU. The only
     function we should expose in a header is u3m_migrate. Call these
     migrate.[c|h] */
static void
_migrate_reclaim()
{
  ...
  */
  ...
  for (; box_w < end_w
         ...
         , box_u = (void*)box_w)
    {
      if (!box_u->use_w)
        continue;
      c3_dessert(box_u->siz_w);
      c3_dessert(box_u->use_w);
      box_w[box_u->siz_w - 1] = new_p;
      /* new_p = c3_align(new_p + box_u->siz_w, 2, ALHI); */
      /* ;;: hand inlined c3_align until that implementation is fixed */
      new_p = (new_p + box_u->siz_w + (2 - 1)) & ~(2 - 1);
    }
}
