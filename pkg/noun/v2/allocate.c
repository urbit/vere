...
void u3a_v1_config_loom(c3_w ver_w);
void *u3a_v1_into(c3_w x);
c3_w u3a_v1_outa(void *p);
c3_w u3a_v1_to_off(c3_w som);
void *u3a_v1_to_ptr(c3_w som);
c3_w *u3a_v1_to_wtr(c3_w som);
c3_w u3a_v1_to_pug(c3_w off);
c3_w u3a_v1_to_pom(c3_w off);

void
u3a_v1_drop(const u3a_v1_pile* pil_u);
void*
u3a_v1_peek(const u3a_v1_pile* pil_u);
void*
u3a_v1_pop(const u3a_v1_pile* pil_u);
void*
u3a_v1_push(const u3a_v1_pile* pil_u);
c3_o
u3a_v1_pile_done(const u3a_v1_pile* pil_u);
...
{
  u3R_v1->all.fre_w += siz_ws;

  {
    c3_w end_w = u3a_v1_heap(u3R_v1);
    c3_w all_w = (end_w - u3R_v1->all.fre_w);

    if ( all_w > u3R_v1->all.max_w ) {
      u3R_v1->all.max_w = all_w;
    }
  }
}
#else
...
#define _box_vaal(box_u)                                                \
  do {                                                                  \
    c3_dessert(((uintptr_t)u3a_v1_boxto(box_u)                             \
                & u3C.balign_d-1) == 0);                                \
    c3_dessert((((u3a_v1_box*)(box_u))->siz_w                              \
                & u3C.walign_w-1) == 0);                                \
  } while(0)
...
_box_slot(c3_w siz_w)
{
  ...
  for (c3_w i_w = 1; i_w < u3a_v1_fbox_no; i_w++) {
    if ( siz_w < 16 ) return i_w;
    siz_w = (siz_w + 1) >> 1;
  }
  return u3a_v1_fbox_no - 1;
}
...
_box_free(u3a_v1_box* box_u)
{
  ...
  */
  ...
  if ( c3y == u3a_v1_is_north(u3R_v1) ) { /* north */
    ...
    */
    if ( box_w != u3a_v1_into(u3R_v1->rut_p) ) {
      ...
      u3a_v1_box* pox_u = (u3a_v1_box*)(void *)(box_w - laz_w); /* the head of the adjacent box below */
    ...
    */
    if ( (box_w + box_u->siz_w) != u3a_v1_into(u3R_v1->rut_p) ) {
      u3a_v1_box* nox_u = (u3a_v1_box*)(void *)(box_w + box_u->siz_w);
    ...
    */
    if ( box_w == u3a_v1_into(u3R_v1->hat_p) ) {
      u3R_v1->hat_p = u3a_v1_outa(box_w + box_u->siz_w);
    }
    else {
      c3_w laz_w = box_w[-1];
...
/* _ca_box_make_hat(): in u3R_v1, allocate directly on the hat.
*/
static u3a_v1_box*
_ca_box_make_hat(c3_w len_w, c3_w ald_w, c3_w off_w, c3_w use_w)
{
  ...
  if ( c3y == u3a_v1_is_north(u3R_v1) ) {
    box_p = all_p = u3R_v1->hat_p;
    all_p += c3_wiseof(u3a_v1_box) + off_w;
    pad_w = c3_align(all_p, ald_w, C3_ALGHI)
      - all_p;
    siz_w = c3_align(len_w + pad_w, u3C.walign_w, C3_ALGHI);

    //  hand-inlined: siz_w >= u3a_v1_open(u3R_v1)
    //
    if ( (siz_w >= (u3R_v1->cap_p - u3R_v1->hat_p)) ) {
      return 0;
    }
    u3R_v1->hat_p += siz_w;
  }
  else {
    box_p = all_p = u3R_v1->hat_p - len_w;
    all_p += c3_wiseof(u3a_v1_box) + off_w;
    pad_w = all_p
    ...
    //  hand-inlined: siz_w >= u3a_v1_open(u3R_v1)
    //
    if ( siz_w >= (u3R_v1->hat_p - u3R_v1->cap_p) ) {
      return 0;
    }
    box_p = u3R_v1->hat_p -= siz_w;
  }
  c3_dessert(!(ald_w <= 2 && off_w == 0) || (0 == pad_w));
  c3_dessert(pad_w <= 4);

  return _box_make(u3a_v1_into(box_p), siz_w, use_w);
}

#if 0
/* _me_road_all_hat(): in u3R_v1, allocate directly on the hat.
*/
static u3a_v1_box*
...
#if 0  // not yet used
/* _me_road_all_cap(): in u3R_v1, allocate directly on the cap.
*/
static c3_w*
_me_road_all_cap(c3_w len_w)
{
  if ( len_w > u3a_v1_open(u3R_v1) ) {
    u3m_bail(c3__meme); return 0;
  }

  if ( c3y == u3a_v1_is_north(u3R_v1) ) {
    u3R_v1->cap_p -= len_w;
    return u3a_v1_into(u3R_v1->cap_p);
  }
  else {
    u3_post all_p;
    ...
    all_p = u3R_v1->cap_p;
    u3R_v1->cap_p += len_w;
    return u3a_v1_into(all_p);
  }
}
#endif

#if 0
/* u3a_v1_sane(): check allocator sanity.
*/
void
u3a_v1_sane(void)
{
  ...
  for ( i_w = 0; i_w < u3a_v1_fbox_no; i_w++ ) {
    u3a_v1_fbox* fre_u = u3R_v1->all.fre_u[i_w];

    while ( fre_u ) {
      if ( fre_u == u3R_v1->all.fre_u[i_w] ) {
...
/* u3a_v1_reflux(): dump 1K cells from the cell list into regular memory.
*/
void
u3a_v1_reflux(void)
{
  ...
  for ( i_w = 0; u3R_v1->all.cel_p && (i_w < 1024); i_w++ ) {
    u3_post  cel_p = u3R_v1->all.cel_p;
    u3a_v1_box* box_u = &(u3to(u3a_v1_fbox, cel_p)->box_u);

    u3R_v1->all.cel_p = u3to(u3a_v1_fbox, cel_p)->nex_p;

    // otherwise _box_free() will double-count it
    //
    _box_count(-(u3a_v1_minimum));
    _box_free(box_u);
...
_ca_reclaim_half(void)
{
  ...
  if ( (0 == u3R_v1->cax.har_p) ||
       (0 == u3to(u3h_root, u3R_v1->cax.har_p)->use_w) )
  {
  ...
  fprintf(stderr, "allocate: reclaim: half of %d entries\r\n",
          u3to(u3h_root, u3R_v1->cax.har_p)->use_w);

  u3h_trim_to(u3R_v1->cax.har_p, u3to(u3h_root, u3R_v1->cax.har_p)->use_w / 2);
#else
  /*  brutal and guaranteed effective
  */
  u3h_free(u3R_v1->cax.har_p);
  u3R_v1->cax.har_p = u3h_new();
#endif
}

/* _ca_willoc(): u3a_v1_walloc() internals.
*/
static void*
_ca_willoc(c3_w len_w, c3_w ald_w, c3_w off_w)
{
  c3_w siz_w = c3_max(u3a_v1_minimum, u3a_v1_boxed(len_w));
  ...
  */
  if ( (sel_w != 0) && (sel_w != u3a_v1_fbox_no - 1) ) {
    sel_w += 1;
  }

  // u3l_log("walloc %d: *pfr_p %x", len_w, u3R_v1->all.fre_p[sel_w]);
  while ( 1 ) {
    u3p(u3a_v1_fbox) *pfr_p = &u3R_v1->all.fre_p[sel_w];
    ...
    while ( 1 ) {
      ...
      if ( 0 == *pfr_p ) {
        if ( sel_w < (u3a_v1_fbox_no - 1) ) {
          ...
          //
          // if ( (u3a_v1_open(u3R_v1) + u3R_v1->all.fre_w) < 65536 ) { _ca_reclaim_half(); }
          ...
          */
          ...
          else return u3a_v1_boxto(box_u);
        }
      }
        ...
        if ( (des_w) > u3to(u3a_v1_fbox, *pfr_p)->box_u.siz_w ) {
          /* This free block is too small.  Continue searching.
          */
          pfr_p = &(u3to(u3a_v1_fbox, *pfr_p)->nex_p);
          continue;
        }
        else {                  /* free block fits desired alloc size */
          u3a_v1_box* box_u = &(u3to(u3a_v1_fbox, *pfr_p)->box_u);
          ...
          */
          ...
          {
            if ( (0 != u3to(u3a_v1_fbox, *pfr_p)->pre_p) &&
                 (u3to(u3a_v1_fbox, u3to(u3a_v1_fbox, *pfr_p)->pre_p)->nex_p
                    != (*pfr_p)) )
            {                   /* this->pre->nex isn't this */
              u3_assert(!"loom: corrupt");
            }

            if( (0 != u3to(u3a_v1_fbox, *pfr_p)->nex_p) &&
                (u3to(u3a_v1_fbox, u3to(u3a_v1_fbox, *pfr_p)->nex_p)->pre_p
                   != (*pfr_p)) )
          ...
          */
          if ( (des_w + u3a_v1_minimum) <= box_u->siz_w ) {
            /* Split the block.
            */
            ...
            return u3a_v1_boxto(_box_make(box_w, des_w, 1));
          }
          else {
...
/* _ca_walloc(): u3a_v1_walloc() internals.
...
_ca_walloc(c3_w len_w, c3_w ald_w, c3_w off_w)
{
  ...
  _box_vaal(u3a_v1_botox(ptr_v));
  return ptr_v;
}

/* u3a_v1_walloc(): allocate storage words on hat heap.
*/
void*
u3a_v1_walloc(c3_w len_w)
{
  void* ptr_v;
...
#if 0
  if ( (703 == u3_Code) &&
      u3a_v1_botox(ptr_v) == (u3a_v1_box*)(void *)0x200dfe3e4 ) {
    static int xuc_i;

    u3l_log("xuc_i %d", xuc_i);
    if ( 1 == xuc_i ) {
      u3a_v1_box* box_u = u3a_v1_botox(ptr_v);
...
/* u3a_v1_wealloc(): realloc in words.
*/
void*
u3a_v1_wealloc(void* lag_v, c3_w len_w)
{
  if ( !lag_v ) {
    return u3a_v1_walloc(len_w);
  }
  else {
    u3a_v1_box* box_u = u3a_v1_botox(lag_v);
    c3_w*    old_w = lag_v;
    c3_w     tiz_w = c3_min(box_u->siz_w, len_w);
    {
      c3_w* new_w = u3a_v1_walloc(len_w);
      c3_w  i_w;
...
/* u3a_v1_pile_prep(): initialize stack control.
*/
void
u3a_v1_pile_prep(u3a_v1_pile* pil_u, c3_w len_w)
{
  //  frame size, in words
  //
  c3_w wor_w = (len_w + 3) >> 2;
  c3_o nor_o = u3a_v1_is_north(u3R_v1);

  pil_u->mov_ws = (c3y == nor_o) ? -wor_w :  wor_w;
  pil_u->off_ws = (c3y == nor_o) ?      0 : -wor_w;
  pil_u->top_p  = u3R_v1->cap_p;
...
u3a_v1_wtrim(void* tox_v, c3_w old_w, c3_w len_w)
{
  ...
  if ( (old_w > len_w)
       && ((old_w - len_w) >= u3a_v1_minimum) )
    {
    u3a_v1_box* box_u = u3a_v1_botox(nov_w);
    c3_w*    box_w = (void*)u3a_v1_botox(nov_w);
...
/* u3a_v1_calloc(): allocate and zero-initialize array
*/
void*
...
u3a_v1_malloc(size_t len_i)
{
  ...
  c3_w  pad_w = u3a_v1_outa(out_w) - u3a_v1_outa(ptr_w);
  ...
  c3_dessert(&out_w[len_w] /* alloced space after alignment is sufficient */
             <= &((c3_w*)u3a_v1_botox(ptr_w))[u3a_v1_botox(ptr_w)->siz_w]);
...
/* u3a_v1_cellblock(): allocate a block of cells on the hat.

   XXX beware when we stop boxing cells and QWORD align references. Alignment
   not guaranteed to be preserved after a call.
*/
static c3_o
u3a_v1_cellblock(c3_w num_w)
{
  u3p(u3a_v1_fbox) fre_p;
  c3_w          i_w;

  if ( c3y == u3a_v1_is_north(u3R_v1) ) {
    if ( u3R_v1->cap_p <= (u3R_v1->hat_p + (num_w * u3a_v1_minimum) + (1 << u3a_v1_page)) ) {
      return c3n;
    }
    else {
      u3_post cel_p = u3R_v1->all.cel_p;
      u3_post hat_p = u3R_v1->hat_p;
      u3R_v1->hat_p   += (num_w * u3a_v1_minimum);

      for ( i_w = 0; i_w < num_w; i_w++) {
        u3_post  all_p = hat_p;
        void*    box_v = u3a_v1_into(all_p);
        u3a_v1_box* box_u = box_v;
        c3_w*    box_w = box_v;

        //  hand inline of _box_make(u3a_v1_into(all_p), u3a_v1_minimum, 1)
        {
          box_u->siz_w = u3a_v1_minimum;
          box_w[u3a_v1_minimum - 1] = u3a_v1_minimum;
          box_u->use_w = 1;
#ifdef U3_MEMORY_DEBUG
            box_u->cod_w = 0;
            box_u->eus_w = 0;
        ...
        hat_p += u3a_v1_minimum;

        fre_p = u3of(u3a_v1_fbox, box_u);
        u3to(u3a_v1_fbox, fre_p)->nex_p = cel_p;
        cel_p = fre_p;
      }

      u3R_v1->all.cel_p = cel_p;
    }
  }
  else {
    if ( (u3R_v1->cap_p + (num_w * u3a_v1_minimum) + (1 << u3a_v1_page)) >= u3R_v1->hat_p ) {
      return c3n;
    }
    else {
      u3_post cel_p = u3R_v1->all.cel_p;
      ...
      for ( i_w = 0; i_w < num_w; i_w++ ) {
        u3_post  all_p = (hat_p -= u3a_v1_minimum);
        void*    box_v = u3a_v1_into(all_p);
        u3a_v1_box* box_u = box_v;
        c3_w*    box_w = box_v;
...
# endif
  ...
  _box_count(num_w * u3a_v1_minimum);
  return c3y;
}

/* u3a_v1_celloc(): allocate a cell.
...
u3a_v1_celloc(void)
{
  ...
  if ( u3C.wag_w & u3o_debug_ram ) {
    return u3a_v1_walloc(c3_wiseof(u3a_v1_cell));
  }
#endif

  u3p(u3a_v1_fbox) cel_p;

  if ( !(cel_p = u3R_v1->all.cel_p) ) {
    if ( u3R_v1 == &(u3H->rod_u) ) {
      ...
      if ( c3n == u3a_v1_cellblock(4096) ) {
        return u3a_v1_walloc(c3_wiseof(u3a_v1_cell));
      }
      cel_p = u3R_v1->all.cel_p;
    }
  }
...
/* u3a_v1_cfree(): free a cell.
*/
void
u3a_v1_cfree(c3_w* cel_w)
{
#ifdef U3_MEMORY_DEBUG
  if ( u3C.wag_w & u3o_debug_ram ) {
    return u3a_v1_wfree(cel_w);
  }
#endif
  ...
  if ( u3R_v1 == &(u3H->rod_u) ) {
    ...
    u3a_v1_box*      box_u = u3a_v1_botox(cel_w);
    u3p(u3a_v1_fbox) fre_p = u3of(u3a_v1_fbox, box_u);

    _box_count(u3a_v1_minimum);

    u3to(u3a_v1_fbox, fre_p)->nex_p = u3R_v1->all.cel_p;
    u3R_v1->all.cel_p = fre_p;
  }
}
...
u3a_v1_realloc(void* lag_v, size_t len_i)
{
  if ( !lag_v ) {
    ...
    u3a_v1_box* box_u = u3a_v1_botox((void *)org_w);
    c3_w*    old_w = lag_v;
    c3_w     tiz_w = c3_min(box_u->siz_w, len_w);
    {
      c3_w* new_w = u3a_v1_malloc(len_i);
      c3_w  i_w;
...
{
  if ( _(u3a_v1_is_cat(som)) ) return;
  if ( !_(u3a_v1_north_is_junior(u3R_v1, som)) ) return;
...
{
  u3_assert(c3y == u3a_v1_is_dog(dog));
  // u3_assert(c3y == u3a_v1_north_is_junior(u3R_v1, dog));
  {
    u3a_v1_noun* dog_u = u3a_v1_to_ptr(dog);
    ...
    if ( _(u3a_v1_is_pom(dog)) ) {
      u3a_v1_cell* god_u = (u3a_v1_cell *)(void *)dog_u;
...
{
  if ( _(u3a_v1_is_cat(som)) ) return;
  if ( !_(u3a_v1_south_is_junior(u3R_v1, som)) ) return;
...
{
  u3_assert(c3y == u3a_v1_is_dog(dog));
  // u3_assert(c3y == u3a_v1_south_is_junior(u3R_v1, dog));
  {
    u3a_v1_noun* dog_u = u3a_v1_to_ptr(dog);
...
/* u3a_v1_wash(): wash all lazy mugs.  RETAIN.
*/
void
u3a_v1_wash(u3_noun som)
{
  if ( _(u3a_v1_is_cat(som)) ) {
    return;
  }
  if ( _(u3a_v1_is_north(u3R_v1)) ) {
    if ( _(u3a_v1_north_is_junior(u3R_v1, som)) ) {
      _me_wash_north(som);
    }
  }
  else {
    if ( _(u3a_v1_south_is_junior(u3R_v1, som)) ) {
      _me_wash_south(som);
    }
  }
}
...
_ca_take_atom(u3a_v1_atom* old_u)
{
  c3_w*     new_w = u3a_v1_walloc(old_u->len_w + c3_wiseof(u3a_v1_atom));
  u3a_v1_atom* new_u = (u3a_v1_atom*)(void *)new_w;
  u3_noun     new = u3a_v1_to_pug(u3a_v1_outa(new_u));

#ifdef VERBOSE_TAKE
  u3l_log("%s: atom %p to %p", ( c3y == u3a_v1_is_north(u3R_v1) )
                                   ? "north"
                                   : "south",
                                   old_u,
                                   new_u);
...
_ca_take_cell(u3a_v1_cell* old_u, u3_noun hed, u3_noun tel)
{
  //  XX use u3a_v1_celloc?
  //
  c3_w*     new_w = u3a_v1_walloc(c3_wiseof(u3a_v1_cell));
  u3a_v1_cell* new_u = (u3a_v1_cell*)(void *)new_w;
  u3_cell     new = u3a_v1_to_pom(u3a_v1_outa(new_u));

#ifdef VERBOSE_TAKE
  u3l_log("%s: cell %p to %p", ( c3y == u3a_v1_is_north(u3R_v1) )
                                   ? "north"
                                   : "south",
                                   old_u,
                                   new_u);
...
_ca_take_next_north(u3a_v1_pile* pil_u, u3_noun veb)
{
  while ( 1 ) {
    //  direct atoms and senior refs are not counted.
    //
    if (  (c3y == u3a_v1_is_cat(veb))
       || (c3y == u3a_v1_north_is_senior(u3R_v1, veb)) )
    {
      return veb;
    }
    //  not junior; normal (heap) refs on our road are counted.
    //
    else if ( c3n == u3a_v1_north_is_junior(u3R_v1, veb) ) {
    ...
    //
      ...
      u3a_v1_noun* veb_u = u3a_v1_to_ptr(veb);
      ...
      if ( veb_u->mug_w >> 31 ) {
        ...
        u3_assert( c3y == u3a_v1_north_is_normal(u3R_v1, nov) );

#ifdef VERBOSE_TAKE
        u3l_log("north: %p is already %p", veb_u, u3a_v1_to_ptr(nov));
#endif

        _me_gain_use(nov); // bypass branches in u3k()
        return nov;
      }
      else if ( c3y == u3a_v1_is_atom(veb) ) {
        return _ca_take_atom((u3a_v1_atom*)veb_u);
      }
      else {
        u3a_v1_cell* old_u = (u3a_v1_cell*)veb_u;
        _ca_take* fam_u = u3a_v1_push(pil_u);
...
*/
static inline u3_noun
_ca_take_next_south(u3a_v1_pile* pil_u, u3_noun veb)
{
  while ( 1 ) {
    //  direct atoms and senior refs are not counted.
    //
    if (  (c3y == u3a_v1_is_cat(veb))
       || (c3y == u3a_v1_south_is_senior(u3R_v1, veb)) )
    {
      return veb;
    }
    //  not junior; a normal pointer in our road -- refcounted
    //
    else if ( c3n == u3a_v1_south_is_junior(u3R_v1, veb) ) {
      _me_gain_use(veb); // bypass branches in u3k()
      return veb;
    }
    //  junior (stack) refs are copied.
    //
      ...
      if ( veb_u->mug_w >> 31 ) {
        ...
        u3_assert( c3y == u3a_v1_south_is_normal(u3R_v1, nov) );

#ifdef VERBOSE_TAKE
        u3l_log("south: %p is already %p", veb_u, u3a_v1_to_ptr(nov));
#endif
...
_ca_take_north(u3_noun veb)
{
  ...
  _ca_take* fam_u;
  u3a_v1_pile  pil_u;
  u3a_v1_pile_prep(&pil_u, sizeof(*fam_u));
  ...
  //
  if ( c3n == u3a_v1_pile_done(&pil_u) ) {
    fam_u = u3a_v1_peek(&pil_u);

    do {
      //  head-frame: stash copy and continue into the tail
      //
      if ( u3_none == fam_u->hed ) {
        u3a_v1_cell* old_u = u3a_v1_to_ptr(fam_u->old);
        fam_u->hed = pro;
      ...
      //
        ...
        fam_u = u3a_v1_pop(&pil_u);
      }
    } while ( c3n == u3a_v1_pile_done(&pil_u) );
  }
...
/* u3a_v1_take(): gain, copying juniors.
*/
u3_noun
u3a_v1_take(u3_noun veb)
{
  u3_noun pro;
  u3t_on(coy_o);
  ...
  pro = ( c3y == u3a_v1_is_north(u3R_v1) )
        ? _ca_take_north(veb)
        : _ca_take_south(veb);
...
/* u3a_v1_left(): true of junior if preserved.
*/
c3_o
u3a_v1_left(u3_noun som)
{
  if ( _(u3a_v1_is_cat(som)) ||
       !_(u3a_v1_is_junior(u3R_v1, som)) )
  {
    return c3y;
  }
  else {
    u3a_v1_noun* dog_u = u3a_v1_to_ptr(som);
...
{
  if ( c3y == u3a_v1_north_is_senior(u3R_v1, dog) ) {
    ...
    */
    u3_assert(!_(u3a_v1_north_is_junior(u3R_v1, dog)));
...
{
  if ( c3y == u3a_v1_south_is_senior(u3R_v1, dog) ) {
    /*  senior pointers are not refcounted
    */
    return dog;
  }
    ...
    /* junior nouns are disallowed
    */
    u3_assert(!_(u3a_v1_south_is_junior(u3R_v1, dog)));
...
{
top:
  if ( c3y == u3a_v1_north_is_normal(u3R_v1, dog) ) {
    c3_w* dog_w      = u3a_v1_to_ptr(dog);
    u3a_v1_box* box_u = u3a_v1_botox(dog_w);
...
{
top:
  if ( c3y == u3a_v1_south_is_normal(u3R_v1, dog) ) {
    c3_w* dog_w      = u3a_v1_to_ptr(dog);
    u3a_v1_box* box_u = u3a_v1_botox(dog_w);
...
/* u3a_v1_gain(): gain a reference count in normal space.
*/
u3_noun
u3a_v1_gain(u3_noun som)
{
  u3t_on(mal_o);
  u3_assert(u3_none != som);

  if ( !_(u3a_v1_is_cat(som)) ) {
    som = _(u3a_v1_is_north(u3R_v1))
              ? _me_gain_north(som)
              : _me_gain_south(som);
  }
  u3t_off(mal_o);
...
/* u3a_v1_use(): reference count.
*/
c3_w
u3a_v1_use(u3_noun som)
{
  if ( _(u3a_v1_is_cat(som)) ) {
    return 1;
  }
  else {
    c3_w*    dog_w = u3a_v1_to_ptr(som);
    u3a_v1_box* box_u = u3a_v1_botox(dog_w);
...
*/
static c3_o
_ca_wed_who(u3a_v1_road* rod_u, u3_noun* a, u3_noun* b)
{
  c3_t asr_t = ( c3y == u3a_v1_is_senior(rod_u, *a) );
  c3_t bsr_t = ( c3y == u3a_v1_is_senior(rod_u, *b) );
  c3_t nor_t = ( c3y == u3a_v1_is_north(rod_u) );
  c3_t own_t = ( rod_u == u3R_v1 );
...
/* u3a_v1_wed(): unify noun references.
*/
void
u3a_v1_wed(u3_noun* a, u3_noun* b)
{
  if ( *a != *b ) {
    u3_v1_road* rod_u = u3R_v1;
            ...
            (c3n == _ca_wed_who(rod_u, a, b)) )
    {
      ...
      break;
#else
      rod_u = u3to(u3_v1_road, rod_u->par_p);
#endif
    }
  }
}

/* u3a_v1_luse(): check refcount sanity.
*/
void
u3a_v1_luse(u3_noun som)
{
  if ( 0 == u3a_v1_use(som) ) {
...
/* u3a_v1_mark_ptr(): mark a pointer for gc.  Produce size if first mark.
*/
c3_w
u3a_v1_mark_ptr(void* ptr_v)
{
  if ( _(u3a_v1_is_north(u3R_v1)) ) {
    if ( !((ptr_v >= u3a_v1_into(u3R_v1->rut_p)) &&
           (ptr_v < u3a_v1_into(u3R_v1->hat_p))) )
    {
      return 0;
    }
  }
  else {
    if ( !((ptr_v >= u3a_v1_into(u3R_v1->hat_p)) &&
           (ptr_v < u3a_v1_into(u3R_v1->rut_p))) )
    {
      return 0;
    }
  }
  {
    ...
    else if ( 0xffffffff == box_u->eus_w ) {      // see u3a_v1_prof()
      siz_w = 0xffffffff;
      box_u->eus_w = 0;
    }
    else {
    ...
    if ( use_ws == 0 ) {
      ...
      if ( 0x80000000 == (c3_w)use_ws ) {    // see u3a_v1_prof()
        use_ws = -1;
        siz_w = 0xffffffff;
      }
      else if ( use_ws < 0 ) {
...
u3_post
u3a_v1_rewritten(u3_post ptr_v)
{
  u3a_v1_box* box_u = u3a_v1_botox(u3a_v1_into(ptr_v));
...
{
  if ( c3y == u3a_v1_is_cat(som) ) {
    return som;
  }
  u3_post som_p = u3a_v1_rewritten(u3a_v1_to_off(som));

  if ( c3y == u3a_v2_is_pug(som) ) {  //  XX alias eventually
...
/* u3a_v1_mark_mptr(): mark a malloc-allocated ptr for gc.
*/
c3_w
u3a_v1_mark_mptr(void* ptr_v)
{
  c3_w* ptr_w = ptr_v;
  c3_w  pad_w = ptr_w[-1];
  c3_w* org_w = ptr_w - (pad_w + 1);

  return u3a_v1_mark_ptr(org_w);
}
...
u3a_v1_mark_noun(u3_noun som)
{
  ...
  while ( 1 ) {
    if ( _(u3a_v1_is_senior(u3R_v1, som)) ) {
      return siz_w;
    }
    else {
      c3_w* dog_w = u3a_v1_to_ptr(som);
      c3_w  new_w = u3a_v1_mark_ptr(dog_w);

      if ( 0 == new_w || 0xffffffff == new_w ) {      //  see u3a_v1_mark_ptr()
        return siz_w;
      }
      else {
        siz_w += new_w;
        if ( _(u3du(som)) ) {
          siz_w += u3a_v1_mark_noun(u3h(som));
          som = u3t(som);
        }
        else return siz_w;
      }
...
/* u3a_v1_count_noun(): count size of pointer.
*/
c3_w
...
/* u3a_v1_count_noun(): count size of noun.
*/
c3_w
u3a_v1_count_noun(u3_noun som)
{
  c3_w siz_w = 0;
  ...
  while ( 1 ) {
    if ( _(u3a_v1_is_senior(u3R_v1, som)) ) {
      ...
      c3_w* dog_w = u3a_v1_to_ptr(som);
      c3_w  new_w = u3a_v1_count_ptr(dog_w);
      ...
      if ( 0 == new_w ) {
        ...
        siz_w += new_w;
        if ( _(u3du(som)) ) {
          siz_w += u3a_v1_count_noun(u3h(som));
          som = u3t(som);
        }
        else return siz_w;
      }
...
/* u3a_v1_discount_ptr(): clean up after counting a pointer.
*/
c3_w
...
u3a_v1_discount_noun(u3_noun som)
{
  ...
  while ( 1 ) {
    if ( _(u3a_v1_is_senior(u3R_v1, som)) ) {
      ...
      c3_w* dog_w = u3a_v1_to_ptr(som);
      c3_w  new_w = u3a_v1_discount_ptr(dog_w);
      ...
      if ( 0 == new_w ) {
        ...
        siz_w += new_w;
        if ( _(u3du(som)) ) {
          siz_w += u3a_v1_discount_noun(u3h(som));
          som = u3t(som);
        }
        else return siz_w;
      }
...
/* u3a_v1_print_time: print microsecond time.
*/
void
u3a_v1_print_time(c3_c* str_c, c3_c* cap_c, c3_d mic_d)
{
  u3_assert( 0 != str_c );
...
/* u3a_v1_print_memory: print memory amount.
*/
void
u3a_v1_print_memory(FILE* fil_u, c3_c* cap_c, c3_w wor_w)
{
  u3_assert( 0 != fil_u );
...
/* u3a_v1_maid(): maybe print memory.
*/
c3_w
u3a_v1_maid(FILE* fil_u, c3_c* cap_c, c3_w wor_w)
{
  if ( 0 != fil_u ) {
    u3a_v1_print_memory(fil_u, cap_c, wor_w);
  }
  return wor_w;
}

/* _ca_print_memory(): un-captioned u3a_v1_print_memory().
*/
static void
...
u3a_v1_prof(FILE* fil_u, c3_w den_w, u3_noun mas)
{
  ...
  if ( c3n == u3r_cell(mas, &h_mas, &t_mas) ) {
    ...
    if ( c3n == u3r_cell(t_mas, &it_mas, &tt_mas) ) {
       ...
       * see u3a_v1_mark_ptr().
      */
      if ( _(u3a_v1_is_dog(tt_mas)) ) {
        u3a_v1_box* box_u = u3a_v1_botox(u3a_v1_to_ptr(tt_mas));
#ifdef U3_MEMORY_DEBUG
        if ( 1 == box_u->eus_w ) {
      ...
      while ( _(u3du(tt_mas)) ) {
        tot_w += u3a_v1_prof(fil_u, den_w+2, u3h(tt_mas));
        tt_mas = u3t(tt_mas);
      }
...
{
  c3_w tot_w = 0;
  tot_w += u3a_v1_maid(fil_u, "  namespace", u3a_v1_mark_noun(u3R_v1->ski.gul));
  tot_w += u3a_v1_maid(fil_u, "  trace stack", u3a_v1_mark_noun(u3R_v1->bug.tax));
  tot_w += u3a_v1_maid(fil_u, "  trace buffer", u3a_v1_mark_noun(u3R_v1->bug.mer));
  tot_w += u3a_v1_maid(fil_u, "  profile batteries", u3a_v1_mark_noun(u3R_v1->pro.don));
  tot_w += u3a_v1_maid(fil_u, "  profile doss", u3a_v1_mark_noun(u3R_v1->pro.day));
  tot_w += u3a_v1_maid(fil_u, "  new profile trace", u3a_v1_mark_noun(u3R_v1->pro.trace));
  tot_w += u3a_v1_maid(fil_u, "  memoization cache", u3h_mark(u3R_v1->cax.har_p));
  return   u3a_v1_maid(fil_u, "total road stuff", tot_w);
}
...
{
  u3a_v1_rewrite_noun(u3R_v1->ski.gul);
  u3a_v1_rewrite_noun(u3R_v1->bug.tax);
  u3a_v1_rewrite_noun(u3R_v1->bug.mer);
  u3a_v1_rewrite_noun(u3R_v1->pro.don);
  u3a_v1_rewrite_noun(u3R_v1->pro.day);
  u3a_v1_rewrite_noun(u3R_v1->pro.trace);
  u3h_rewrite(u3R_v1->cax.har_p);

  u3R_v1->ski.gul = u3a_v1_rewritten_noun(u3R_v1->ski.gul);
  u3R_v1->bug.tax = u3a_v1_rewritten_noun(u3R_v1->bug.tax);
  u3R_v1->bug.mer = u3a_v1_rewritten_noun(u3R_v1->bug.mer);
  u3R_v1->pro.don = u3a_v1_rewritten_noun(u3R_v1->pro.don);
  u3R_v1->pro.day = u3a_v1_rewritten_noun(u3R_v1->pro.day);
  u3R_v1->pro.trace = u3a_v1_rewritten_noun(u3R_v1->pro.trace);
  u3R_v1->cax.har_p = u3a_v1_rewritten(u3R_v1->cax.har_p);
}

/* _ca_print_box(): heuristically print the contents of an allocation box.
*/
static c3_c*
_ca_print_box(u3a_v1_box* box_u)
{
  //  the loom offset pointing to the contents of box_u
  //
  c3_w box_w = u3a_v1_outa(u3a_v1_boxto(box_u));
  //  box_u might not be a cell, we use the struct to inspect further
  //
  u3a_v1_cell* cel_u = (u3a_v1_cell*)box_u;

  if (  //  a cell will never be bigger than the minimum allocation size
        //
        (u3a_v1_minimum < box_u->siz_w) ||
          ...
          (0x80000000 & (c3_w)cel_u->tel) ) )
  {
    ...
    //
    u3a_v1_atom* vat_u = (u3a_v1_atom*)box_u;
    u3_atom   veb   = u3a_v1_to_pug(box_w);
    ...
    //
    return u3m_pretty(u3a_v1_to_pom(box_w));
  }
}
...
static void
_ca_print_leak(c3_c* cap_c, u3a_v1_box* box_u, c3_w eus_w, c3_w use_w)
{
  fprintf(stderr, "%s: %p mug=%x (marked=%u swept=%u)\r\n",
                  cap_c,
                  box_u,
                  ((u3a_v1_noun *)(u3a_v1_boxto(box_u)))->mug_w,
                  eus_w,
                  use_w);
  ...
  u3a_v1_print_memory(stderr, "    size", box_u->siz_w);
...
static void
_ca_print_leak(c3_c* cap_c, u3a_v1_box* box_u, c3_ws use_ws)
{
...
/* u3a_v1_idle(): measure free-lists in [rod_u]
*/
c3_w
u3a_v1_idle(u3a_v1_road* rod_u)
{
  c3_w i_w, fre_w = 0;

  for ( i_w = 0; i_w < u3a_v1_fbox_no; i_w++ ) {
    u3p(u3a_v1_fbox) fre_p = rod_u->all.fre_p[i_w];

    while ( fre_p ) {
      u3a_v1_fbox* fox_u = u3to(u3a_v1_fbox, fre_p);
...
/* u3a_v1_sweep(): sweep a fully marked road.
*/
c3_w
u3a_v1_sweep(void)
{
  ...
  */
  {
    c3_w end_w = u3a_v1_heap(u3R_v1);
    c3_w fre_w = u3a_v1_idle(u3R_v1);

#ifdef U3_CPU_DEBUG
    if ( fre_w != u3R_v1->all.fre_w ) {
      fprintf(stderr, "fre discrepancy (%x): %x, %x, %x\r\n", u3R_v1->par_p,
                      fre_w, u3R_v1->all.fre_w, (u3R_v1->all.fre_w - fre_w));
    }
#endif
    neg_w = (end_w - fre_w);
  }
  ...
  */
  pos_w = leq_w = weq_w = 0;
  {
    u3_post box_p = _(u3a_v1_is_north(u3R_v1)) ? u3R_v1->rut_p : u3R_v1->hat_p;
    u3_post end_p = _(u3a_v1_is_north(u3R_v1)) ? u3R_v1->hat_p : u3R_v1->rut_p;
    c3_w*   box_w = u3a_v1_into(box_p);
    c3_w*   end_w = u3a_v1_into(end_p);

    while ( box_w < end_w ) {
      u3a_v1_box* box_u = (void *)box_w;

#ifdef U3_MEMORY_DEBUG
      /* I suspect these printfs fail hilariously in the case
       * of non-direct atoms. We shouldn't unconditionally run
       * u3a_v1_to_pom(). In general, the condition
       * box_u->siz_w > u3a_v1_minimum is sufficient, but not necessary,
...
#ifdef U3_MEMORY_DEBUG
  {
    c3_w tot_w = u3a_v1_full(u3R_v1);
    c3_w caf_w = u3a_v1_temp(u3R_v1);

#ifdef U3_CPU_DEBUG
    if ( (0 != u3R_v1->par_p) && (u3R_v1->all.max_w > 1000000) ) {
      u3a_v1_print_memory(stderr, "available", (tot_w - pos_w));
      u3a_v1_print_memory(stderr, "allocated", pos_w);
      u3a_v1_print_memory(stderr, "volatile", caf_w);

      u3a_v1_print_memory(stderr, "maximum", u3R_v1->all.max_w);
    }
#endif
  ...
  u3a_v1_print_memory(stderr, "leaked", leq_w);
  u3a_v1_print_memory(stderr, "weaked", weq_w);
...
/* u3a_v1_pack_seek(): sweep the heap, modifying boxes to record new addresses.
*/
void
u3a_v1_pack_seek(u3a_v1_road* rod_u)
{
  ...
  //
  c3_w*    box_w = u3a_v1_into(rod_u->rut_p);
  c3_w*    end_w = u3a_v1_into(rod_u->hat_p);
  u3_post  new_p = (rod_u->rut_p + c3_wiseof(u3a_v1_box));
  u3a_v1_box* box_u;
  c3_w     siz_w;

  if ( c3y == u3a_v1_is_north(rod_u) ) {
...
_ca_pack_move_north(c3_w* box_w, c3_w* end_w, u3_post new_p)
{
  ...
  while ( box_w < end_w ) {
    ...
    if ( old_u->use_w ) {
      c3_w* new_w = (void*)u3a_v1_botox(u3a_v1_into(new_p));
...
/* u3a_v1_pack_move(): sweep the heap, moving boxes to new addresses.
*/
void
u3a_v1_pack_move(u3a_v1_road* rod_u)
{
  //  box_w: front of the heap
  //  end_w: back of the heap
  //  new_p: initial new location (data of first box)
  ...
  c3_w*   end_w = u3a_v1_into(rod_u->hat_p);
  u3_post new_p = (rod_u->rut_p + c3_wiseof(u3a_v1_box));
  u3_post las_p = ( c3y == u3a_v1_is_north(rod_u) )
                  ? _ca_pack_move_north(box_w, end_w, new_p)
                  : _ca_pack_move_south(box_w, end_w, new_p);

  rod_u->hat_p  = (las_p - c3_wiseof(u3a_v1_box));
...
/* u3a_v1_rewrite_ptr(): mark a pointer as already having been rewritten
*/
c3_o
u3a_v1_rewrite_ptr(void* ptr_v)
{
  u3a_v1_box* box_u = u3a_v1_botox(ptr_v);
...
{
  if ( c3n == u3a_v1_is_cell(som) ) {
    return;
  }

  if ( c3n == u3a_v1_rewrite_ptr(u3a_v1_to_ptr((som))) ) return;

  u3a_v1_cell* cel = u3a_v1_to_ptr(som);
...
/* u3a_v1_detect(): for debugging, check if (som) is referenced from (fum).
**
** (som) and (fum) are both RETAINED.
*/
c3_d
u3a_v1_detect(u3_noun fum, u3_noun som)
{
...
#ifdef U3_MEMORY_DEBUG
/* u3a_v1_lush(): leak push.
*/
c3_w
u3a_v1_lush(c3_w lab_w)
{
  c3_w cod_w = u3_Code;
              ...
              c3_o     (*cel_f)(u3_noun, void*))
{
  ...
  //
  u3a_v1_pile_prep(&pil_u, sizeof(u3_noun));
  top  = u3a_v1_push(&pil_u);
  *top = a;

  while ( c3n == u3a_v1_pile_done(&pil_u) ) {
    //  visit an atom, then pop the stack
    //
    if ( c3y == u3a_v1_is_atom(a) ) {
      pat_f(a, ptr_v);
      top = u3a_v1_pop(&pil_u);
    }
...
/* u3a_v1_string(): `a` as an on-loom c-string.
*/
c3_c*
u3a_v1_string(u3_atom a)
{
  c3_w  met_w = u3r_met(3, a);
  c3_c* str_c = u3a_v1_malloc(met_w + 1);
...
/* u3a_v1_loom_sane(): sanity checks the state of the loom for obvious corruption
 */
void
u3a_v1_loom_sane()
{
  ...
  */
  for (c3_w i_w = 0; i_w < u3a_v1_fbox_no; i_w++) {
    u3p(u3a_v1_fbox) this_p = u3R_v1->all.fre_p[i_w];
    u3a_v1_fbox     *this_u = u3to(u3a_v1_fbox, this_p);
    for (; this_p
           ; this_p = this_u->nex_p
           , this_u = u3to(u3a_v1_fbox, this_p)) {
      u3p(u3a_v1_fbox) pre_p = this_u->pre_p
        ,           nex_p = this_u->nex_p;
      u3a_v1_fbox *pre_u = u3to(u3a_v1_fbox, this_u->pre_p)
        ,      *nex_u = u3to(u3a_v1_fbox, this_u->nex_p);
      ...
      if (!pre_p                /* this must be the head of a freelist */
          && u3R_v1->all.fre_p[_box_slot(this_u->box_u.siz_w)] != this_p)
        u3_assert(!"loom: wack");
    }
  }
}
