...
{
  u3h_root* har_u = u3a_walloc(c3_wiseof(u3h_root) + u3h_size_min);
  har_u->use_w = 0;
  har_u->loc_w = u3h_size_min;
  har_u->max_w = max_w;
  memset(har_u->kev, 0, u3h_size_min * sizeof(c3_w));
...
_h_expand_table(u3h_root** har_u)
{
  ...
  if ( loc_w > (*har_u)->max_w ) return c3n;
  u3h_root* new_u = u3a_walloc(c3_wiseof(u3h_root) + loc_w);
  new_u->use_w = 0;
  new_u->loc_w = loc_w;
  new_u->max_w = (*har_u)->max_w;
  memset(new_u->kev, 0, loc_w * sizeof(c3_w));
  u3h_uni(u3of(u3h_root, new_u), u3of(u3h_root, *har_u));
  u3a_wfree(*har_u);
  *har_u = new_u;
  return c3y;
}
              ...
              void* wit)
{
  ...
  for (c3_w i_w = 0; i_w < har_u->loc_w; i_w++) {
    if ( har_u->kev[i_w] ) fun_f(har_u->kev[i_w], wit);
  }
}
...
u3h_put(u3p(u3h_root) har_p, u3_noun key, u3_noun val)
{
  ...
  if ( c3n == del_o ) {
    c3_w inc_w = 1;
    while ( (old = har_u->kev[idx_w]) && c3n == u3r_sing(key, _h_hed(old)) ) {
...
u3h_git(u3p(u3h_root) har_p, u3_noun key)
{
  ...
  if ( 0 == har_u->kev[idx_w] ) return u3_none;
  return _h_tel(har_u->kev[idx_w]);
}
...
u3h_trim_to(u3p(u3h_root) har_p, c3_w n_w)
{
  ...
  while ( har_u->use_w > n_w ) {
    ...
    while ( 0 == (old = har_u->kev[idx_w]) ) idx_w++;
    har_u->kev[idx_w] = 0;
    u3z(old);
    har_u->use_w--;
  }
...
u3h_mark(u3p(u3h_root) har_p)
{
  ...
  u3h_root* har_u = u3to(u3h_root, har_p);
  for (c3_w i_w = 0; i_w < har_u->loc_w; i_w++) {
    tot_w += u3a_mark_noun(har_u->kev[i_w]);
  }
...
u3h_relocate(u3p(u3h_root) *har_p)
{
  ...
  *har_p = new_p;
  ...
  for (c3_w i_w = 0; i_w < har_u->loc_w; i_w++) {
    if ( har_u->kev[i_w] ) {
      u3a_relocate_noun(&har_u->kev[i_w]);
    }
  }
}
