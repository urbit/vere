...
*/
#define CUT_END(a_w, b_w) ((a_w) & (((c3_w_new)1 << (b_w)) - 1))

/* BIT_SET(): [1] if bit [b_w] is set in [a_w]
*/
#define BIT_SET(a_w, b_w) ((a_w) & ((c3_w_new)1 << (b_w)))

static u3_weak
_ch_trim_slot(u3h_root* har_u, u3h_slot *sot_n, c3_w_new lef_w, c3_w_new rem_w);

static u3_weak
_ch_trim_root(u3h_root* har_u);

c3_w_new
_ch_skip_slot(c3_w_new mug_w, c3_w_new lef_w);

/* u3h_new_cache(): create hashtable with bounded size.
*/
u3p(u3h_root)
u3h_new_cache(c3_n max_w)
{
...
static void _ch_slot_put(u3h_slot*, u3_noun, c3_w_new, c3_w_new, c3_n*);

/* _ch_node_add(): add to node.
*/
static u3h_node*
_ch_node_add(u3h_node* han_u, c3_w_new lef_w, c3_w_new rem_w, u3_noun kev, c3_n *use_w)
{
  c3_w_new bit_w, inx_w, map_w, i_w;
  ...
  if ( BIT_SET(map_w, bit_w) ) {
    ...
    //
    c3_w_new      len_w = _ch_popcount(map_w);
    u3h_node* nah_u = _ch_node_new(1 + len_w);
    nah_u->map_w    = han_u->map_w | ((c3_w_new)1 << bit_w);
...
*/
static u3h_buck*
_ch_buck_add(u3h_buck* hab_u, u3_noun kev, c3_n *use_w)
{
  c3_w_new i_w;
...
*/
static void*
_ch_some_add(void* han_v, c3_w_new lef_w, c3_w_new rem_w, u3_noun kev, c3_n *use_w)
{
  if ( 0 == lef_w ) {
...
*/
u3h_slot
_ch_two(u3h_slot had_w, u3h_slot add_w, c3_w_new lef_w, c3_w_new ham_w, c3_w_new mad_w)
{
  void* ret;
  ...
  if ( 0 == lef_w ) {
    ...
    if ( hop_w == tad_w ) {
      ...
      ret             = han_u;
      han_u->map_w    = (c3_w_new)1 << hop_w;
      ham_w           = CUT_END(ham_w, lef_w);
      mad_w           = CUT_END(mad_w, lef_w);
      han_u->sot_n[0] = _ch_two(had_w, add_w, lef_w, ham_w, mad_w);
    }
    else {
      u3h_node* han_u = _ch_node_new(2);
      ret             = han_u;
      han_u->map_w    = ((c3_w_new)1 << hop_w) | ((c3_w_new)1 << tad_w);
...
void _hbreak() {}
static void
_ch_slot_put(u3h_slot* sot_n, u3_noun kev, c3_w_new lef_w, c3_w_new rem_w, c3_n* use_w)
{
    ...
    *sot_n = u3h_node_to_slot(hav_v);
    ...
    if ( c3y == u3r_sing(u3h(kev), u3h(kov)) ) {
      ...
      *sot_n = add_w;
      ...
      c3_w_new ham_w = CUT_END(u3r_mug(u3h(kov)), lef_w);
      *sot_n     = _ch_two(*sot_n, add_w, lef_w, ham_w, rem_w);
      *use_w    += 1;
    }
    _hbreak();
  }
...
u3h_put_get(u3p(u3h_root) har_p, u3_noun key, u3_noun val)
{
  ...
  c3_w_new      mug_w = u3r_mug(key);
  c3_w_new      inx_w = (mug_w >> 25);  //  6 bits
  c3_w_new      rem_w = CUT_END(mug_w, 25);
...
static c3_o _ch_some_del(u3h_slot*, u3_noun, c3_w_new, c3_w_new);

/* _ch_slot_del(): delete from slot
*/
static c3_o
_ch_slot_del(u3h_slot* sot_n, u3_noun key, c3_w_new lef_w, c3_w_new rem_w)
{
...
*/
static c3_o
_ch_node_del(u3h_slot* sot_n, u3_noun key, c3_w_new lef_w, c3_w_new rem_w)
{
  ...
  if ( c3n == _ch_slot_del(tos_n, key, lef_w, rem_w) ) {
    ...
    // shrink!
    c3_w_new i_w, ken_w, len_w = _ch_popcount(map_w);
    u3h_slot  kes_w;

    if ( 2 == len_w && ((ken_w = (0 == inx_w) ? 1 : 0),
                        (kes_w = han_u->sot_n[ken_w]),
...
*/
static c3_o
_ch_some_del(u3h_slot* sot_n, u3_noun key, c3_w_new lef_w, c3_w_new rem_w)
{
  if ( 0 == lef_w ) {
...
*/
static u3_weak
_ch_trim_node(u3h_root* har_u, u3h_slot* sot_n, c3_w_new lef_w, c3_w_new rem_w)
{
  c3_w_new bit_w, map_w, inx_w;
  u3h_slot* tos_n;
  u3h_node* han_u = (u3h_node*) u3h_slot_to_node(*sot_n);
...
*/
static u3_weak
_ch_trim_some(u3h_root* har_u, u3h_slot* sot_n, c3_w_new lef_w, c3_w_new rem_w)
{
  if ( 0 == lef_w ) {
...
_ch_skip_slot(c3_w_new mug_w, c3_w_new lef_w)
{
  c3_w_new hig_w = mug_w >> lef_w;
  c3_w_new new_w = CUT_END(hig_w + 1, (31 - lef_w)); // modulo 2^(31 - lef_w)
  return new_w << lef_w;
}
...
_ch_trim_root(u3h_root* har_u)
{
  c3_w_new      mug_w = har_u->arm_u.mug_w;
  c3_w_new      inx_w = mug_w >> 25; // 6 bits
  u3h_slot* sot_n = &(har_u->sot_n[inx_w]);
...
*/
static c3_o
_ch_buck_hum(u3h_buck* hab_u, c3_w_new mug_w)
{
  c3_w_new i_w;
...
*/
static c3_o
_ch_node_hum(u3h_node* han_u, c3_w_new lef_w, c3_w_new rem_w, c3_w_new mug_w)
{
  c3_w_new bit_w, map_w;
  ...
  if ( !BIT_SET(map_w, bit_w) ) {
    ...
    c3_w_new inx_w = _ch_popcount(CUT_END(map_w, bit_w));
    u3h_slot sot_n = han_u->sot_n[inx_w];
...
*/
c3_o
u3h_hum(u3p(u3h_root) har_p, c3_w_new mug_w)
{
  u3h_root* har_u = u3to(u3h_root, har_p);
  c3_w_new      inx_w = (mug_w >> 25);
  c3_w_new      rem_w = CUT_END(mug_w, 25);
...
*/
static u3_weak
_ch_node_git(u3h_node* han_u, c3_w_new lef_w, c3_w_new rem_w, u3_noun key)
{
  c3_w_new bit_w, map_w;
...
*/
static void
_ch_free_node(u3h_node* han_u, c3_w_new lef_w, c3_o pin_o)
{
  c3_w_new len_w = _ch_popcount(han_u->map_w);
  c3_w_new i_w;

  lef_w -= 5;

  for ( i_w = 0; i_w < len_w; i_w++ ) {
    u3h_slot sot_n = han_u->sot_n[i_w];
    if ( _(u3h_slot_is_null(sot_n))) {
    }  else if ( _(u3h_slot_is_noun(sot_n)) ) {
...
u3h_free(u3p(u3h_root) har_p)
{
  ...
  for ( i_w = 0; i_w < 64; i_w++ ) {
    u3h_slot sot_n = har_u->sot_n[i_w];
...
*/
static void
_ch_walk_node(u3h_node* han_u, c3_w_new lef_w, void (*fun_f)(u3_noun, void*), void* wit)
{
  c3_w_new len_w = _ch_popcount(han_u->map_w);
  c3_w_new i_w;
...
*/
static u3h_slot
_ch_take_node(u3h_slot sot_n, c3_w_new lef_w, u3_funk fun_f)
{
  u3h_node* han_u = u3h_slot_to_node(sot_n);
  c3_w_new      len_w = _ch_popcount(han_u->map_w);
  ...
  for ( i_w = 0; i_w < len_w; i_w++ ) {
    u3h_slot    tos_n = han_u->sot_n[i_w];
...
_ch_mark_buck(u3h_buck* hab_u)
{
  ...
  for ( i_w = 0; i_w < hab_u->len_w; i_w++ ) {
    tot_w += u3a_mark_noun(u3h_slot_to_noun(hab_u->sot_n[i_w]));
  }
...
*/
c3_w_new
_ch_mark_node(u3h_node* han_u, c3_w_new lef_w)
{
  c3_w_new tot_w = 0;
  c3_w_new len_w = _ch_popcount(han_u->map_w);
  c3_w_new i_w;
...
static void
_ch_relocate_noun(u3h_slot *sot_n)
{
  u3_noun kev = u3h_slot_to_noun(*sot_n);
  u3a_relocate_noun(&kev);
  *sot_n = u3h_noun_to_slot(kev);
}

static void
_ch_relocate_buck(u3h_slot *sot_n)
{
  u3h_buck *hab_u = u3h_slot_to_node(*sot_n);
  ...
  *sot_n = u3h_node_to_slot(u3a_into(new_p));

  if ( !fir_t ) return;

  for ( c3_w_new i_w = 0; i_w < hab_u->len_w; i_w++ ) {
    _ch_relocate_noun(&(hab_u->sot_n[i_w]));
  }
}

static void
_ch_relocate_slot(u3h_slot *sot_n, c3_w_new lef_w);

static void
_ch_relocate_node(u3h_slot *sot_n, c3_w_new lef_w)
{
  u3h_node* han_u = u3h_slot_to_node(*sot_n);
  ...
  *sot_n = u3h_node_to_slot(u3a_into(new_p));
  ...
  for ( c3_w_new i_w = 0; i_w < len_w; i_w++ ) {
    _ch_relocate_slot(&(han_u->sot_n[i_w]), lef_w);
  }
}
...
{
  if ( c3y == u3h_slot_is_noun(*sot_n) ) {
    _ch_relocate_noun(sot_n);
  }
  else if ( !lef_w ) {
    _ch_relocate_buck(sot_n);
  }
  else {
    _ch_relocate_node(sot_n, lef_w);
  }
}
...
u3h_relocate(u3p(u3h_root) *har_p)
{
  ...
  *har_p = new_p;
  ...
  for ( i_w = 0; i_w < 64; i_w++ ) {
    ...
    if ( c3n == u3h_slot_is_null(sot_n) ) {
      _ch_relocate_slot(&(har_u->sot_n[i_w]), 25);
    }
  }
}
...
_ch_count_buck(u3h_buck* hab_u)
{
  ...
  for ( i_w = 0; i_w < hab_u->len_w; i_w++ ) {
    tot_w += u3a_count_noun(u3h_slot_to_noun(hab_u->sot_n[i_w]));
  }
...
*/
c3_w_new
_ch_count_node(u3h_node* han_u, c3_w_new lef_w)
{
  c3_w_new tot_w = 0;
  c3_w_new len_w = _ch_popcount(han_u->map_w);
  c3_w_new i_w;
...
_ch_discount_buck(u3h_buck* hab_u)
{
  ...
  for ( i_w = 0; i_w < hab_u->len_w; i_w++ ) {
    tot_w += u3a_discount_noun(u3h_slot_to_noun(hab_u->sot_n[i_w]));
  }
...
*/
c3_w_new
_ch_discount_node(u3h_node* han_u, c3_w_new lef_w)
{
  c3_w_new tot_w = 0;
  c3_w_new len_w = _ch_popcount(han_u->map_w);
  c3_w_new i_w;
