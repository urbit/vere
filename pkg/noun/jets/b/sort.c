...
//  allocation size check: max list length * 2 * sizeof(u3_noun) must not overflow
//  (we allocate an array and a temp buffer, hence the factor of 2)
//
static_assert(
  (u3a_cells < (SIZE_MAX / (2 * sizeof(u3_noun)))),
  "u3a_cells * 2 * sizeof(u3_noun) overflow"
);
