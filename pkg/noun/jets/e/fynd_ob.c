...
//  (flop raku:ob)
//
static const c3_w_new kar_w[4] = { 0x4b387af7, 0x85bcae01, 0xee281300, 0xb76d5eed };

/* _fen_ob(): +fen:ob, with constant parameters factored out.
**           correct over the domain [0x0 ... 0xfffe.ffff]
*/
static c3_w_new
_fen_ob(c3_w_new m_w)
{
...
*/
static c3_w_new
_tail_ob(c3_w_new m_w)
{
  c3_w_new c_w = _fen_ob(m_w - b_w);
...
u3qe_fynd_ob(u3_atom pyn)
{
  ...
  if ( 2 == sor_w ) {
    return u3i_word_new(_tail_ob(u3r_word_new(0, pyn)));
  }
  else {
    c3_w_new pyn_w[2];
