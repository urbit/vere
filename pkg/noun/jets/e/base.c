...
static inline c3_o
_of_hex_odd(u3_atom inp, c3_n len_n, c3_n byt_w, c3_y* buf_y)
{
...
static inline c3_o
_of_hex_even(u3_atom inp, c3_n len_n, c3_y* buf_y)
{
  c3_y lit_y, hit_y;
  c3_s inp_s;

  while ( len_n-- ) {
    inp_s = u3r_short(len_n, inp);
...
u3qe_de_base16(u3_atom inp)
{
  ...
  //
  {
    c3_n byt_w = u3r_met(3, inp);
    c3_o ret_o = ( byt_w & 1 )
               ? _of_hex_odd(inp, len_n, byt_w, sab_u.buf_y)
               : _of_hex_even(inp, len_n, sab_u.buf_y);
    ...
    if ( c3n == ret_o ) {
      ...
      return u3nt(u3_nul, u3i_note(len_n), dat);
    }
  }
}
