        ...
        )
{
  ...
  if ( _(u3a_is_cat(e)) ) {
    ...
    len_src_w = src_u->len_n;
    src_w = src_u->buf_n;
  }
  u3i_slab_init(&sab_u, a_g, c3_max(len_e_w, b_w + c_w));
  u3r_chop_notes(a_g, 0, b_w, 0, sab_u.buf_n, len_src_w, src_w);
  u3r_chop(a_g, 0, c_w, b_w, sab_u.buf_n, d);
  if (len_e_w > b_w + c_w) {
    u3r_chop_notes(a_g,
             b_w + c_w,
             len_e_w - (b_w + c_w),
