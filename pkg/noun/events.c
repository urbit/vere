                     ...
                     c3_w*        mem_w)
{
  ...
  {
    if ( 0 < ret_zs ) {
      fprintf(stderr, "loom: patch partial write: %"PRIc3_zs", you likely ran out of disk \r\n", ret_zs);
    }
    else {
      fprintf(stderr, "loom: patch write: fail: %s, you likely ran out of disk\r\n", strerror(errno));
    }
    u3_assert(0);
  }
}
    ...
    if ( _ce_page != (ret_zs = read(pat_u->mem_i, buf_y, _ce_page)) ) {
           ...
           (ret_zs = pwrite(fid_i, buf_y, _ce_page, off_z)) )
      {
        if ( 0 < ret_zs ) {
          ...
          fprintf(stderr, "loom: patch apply write: %s, you likely ran out of disk\r\n", strerror(errno));
        }
        u3_assert(0);
      }
    }
