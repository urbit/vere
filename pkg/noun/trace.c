...
u3t_sstack_init(c3_d* who_d)
{
...
#ifndef U3_OS_windows
  c3_h shm_fd = shm_open(_spin_nam_c, O_CREAT | O_RDWR, 0666);
  if ( -1 == shm_fd) {
  ...
  //
  u3H->rod_u.off_h = u3t_Spin->off_h;
  u3H->rod_u.fow_h = u3t_Spin->fow_h;
}
...
u3t_sstack_open(c3_d* who_d)
{
  ...
  //
  c3_h shm_fd = shm_open(_spin_nam_c, O_RDONLY, 0);
  if ( -1 == shm_fd) {
    perror("shm_open failed");
    return NULL;
  }
...
{
  __atomic_store_n(&u3t_Spin->seq_h, u3t_Spin->seq_h + 1, __ATOMIC_RELAXED);
...
u3t_sstack_push(u3_noun nam)
{
  ...
  c3_w met_w = u3r_met_w(3, nam);
  u3_assert( met_w < 0xfffffff );
...
u3t_sstack_pop()
{
  ...
  c3_h len_h;
  memcpy(&len_h, &u3t_Spin->dat_y[u3t_Spin->off_h - sizeof(c3_h)],
                 sizeof(c3_h));

  //  guard against underflow
  //
  if ( (len_h + sizeof(c3_h)) > u3t_Spin->off_h ) {
    fprintf(stderr, "spin: would underflow off=%u siz=%u\r\n",
                    u3t_Spin->off_h, len_h + (c3_h)sizeof(c3_h));
    return;
  }
