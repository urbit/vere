/// @file

#ifndef U3_OPTIONS_H
#define U3_OPTIONS_H

#include "c3.h"
#include "types.h"

  /** Data structures.
  **/
    /* u3o_config: process / system configuration.
    */
      typedef struct _u3o_config {
        u3_noun who;                          //  single identity
        c3_c*   dir_c;                        //  execution directory (pier)
        c3_w    wag_w;                        //  flags (both ways)
        /* ;;: these new members should be relocated to a new struct in a
             migrate.h header -- or should they? Given that wor_i is located
             here, these also being here seems fine. Otherwise, maybe wor_i,
             vit_w, walign_w, balign_d, and migration_state should be moved to a
             loom-specific configuration struct */
        c3_w    vits_w;                       //  number of virtual bits in reference
        c3_w    walign_w;                     //  word alignment
        c3_d    balign_d;                     //  byte alignment
        enum {
          MIG_NONE,
          MIG_REWRITE_COMPRESSED,
        } migration_state;

        size_t  wor_i;                        //  loom word-length (<= u3a_words)
        void (*stderr_log_f)(c3_c*);          //  errors from c code
        void (*slog_f)(u3_noun);              //  function pointer for slog
        void (*sign_hold_f)(void);            //  suspend system signal regime
        void (*sign_move_f)(void);            //  restore system signal regime
      } u3o_config;

    /* u3o_flag: process/system flags.
    **
    **  _debug flags are set outside u3 and heard inside it.
    **  _check flags are set inside u3 and heard outside it.
    */
      enum u3o_flag {                         //  execution flags
        u3o_debug_ram =     0x1,              //  debug: gc
        u3o_debug_cpu =     0x2,              //  debug: profile
        u3o_check_corrupt = 0x4,              //  check: gc memory
        u3o_check_fatal =   0x8,              //  check: unrecoverable
        u3o_verbose =       0x10,             //  be remarkably wordy
        u3o_dryrun =        0x20,             //  don't touch checkpoint
        u3o_quiet =         0x40,             //  disable ~&
        u3o_hashless =      0x80,             //  disable hashboard
        u3o_trace =         0x100             //  enables trace dumping
      };

  /** Globals.
  **/
    /* u3_Config / u3C: global memory control.
    */
      extern u3o_config u3o_Config;
#     define u3C u3o_Config

#endif /* ifndef U3_OPTIONS_H */
