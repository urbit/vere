...
_mars_post(u3_mars* mar_u)
{
  if ( mar_u->fag_w & _mars_fag_hit1 ) {
    if ( u3C.wag_w & u3o_verbose ) {
      u3l_log("mars: threshold 1: %u", u3h_wyt(&u3R->cax.per_u));
    }
    u3h_trim_to(&u3R->cax.per_u, u3h_wyt(&u3R->cax.per_u) / 2);
    u3m_reclaim();
  }
  ...
  if ( mar_u->fag_w & _mars_fag_hit0 ) {
    if ( u3C.wag_w & u3o_verbose ) {
      u3l_log("mars: threshold 0: per_p %u", u3h_wyt(&u3R->cax.per_u));
    }
