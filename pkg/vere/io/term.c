...
_term_it_send_csi(u3_utty *uty_u, c3_c cmd_c, c3_w num_w, ...)
{
  ...
  while ( num_w-- ) {
    ...
    len_y += sprintf(pas_c+len_y, "%"PRIc3_w, par_w);
...
{
  c3_w row_l = uty_u->tat_u.siz.row_l;
  c3_w col_l = uty_u->tat_u.siz.col_l;
...
{
  c3_c str_c[4];
  c3_w len = snprintf(str_c, 4, "%"PRIc3_w, val % 256);
  for ( c3_w i_w = 0ULL; i_w < len; i_w++ ) {
    lin_w[i_w] = str_c[i_w];
  }
  u3z(val);
  return len;
 ...
 */
static u3_noun
_reck_orchid(u3_noun fot, u3_noun txt, c3_w* tid_l)
{
      ...
      switch ( tag ) {
        ...
        case c3__blit: {
...

          {
            u3_utty* uty_u = _term_ef_get(tid_l);
            if ( 0 == uty_u ) {
              // u3l_log("no terminal %"PRIc3_w, tid_l);
