...
void bitset_init(u3_bitset* bit_u, c3_w_tmp len_w, arena* are_u)
{
  bit_u->len_w = len_w;
...
void bitset_put(u3_bitset* bit_u, c3_w_tmp mem_w)
{
  if (( mem_w > bit_u->len_w )) {
    u3l_log("overrun %u, %u", mem_w, bit_u->len_w);
    return;
  }
  c3_w_tmp idx_w = mem_w >> 3;
  c3_w_tmp byt_y = bit_u->buf_y[idx_w];
...
c3_o
bitset_has(u3_bitset* bit_u, c3_w_tmp mem_w) {
  if (( mem_w > bit_u->len_w )) {
    u3l_log("overrun %u, %u", mem_w, bit_u->len_w);
    return c3n;
  }
...
void
bitset_del(u3_bitset* bit_u, c3_w_tmp mem_w)
{
  u3_assert( mem_w < bit_u->len_w );
  c3_w_tmp idx_w = mem_w >> 3;
  c3_w_tmp byt_y = bit_u->buf_y[idx_w];
...
c3_w_tmp main()
{
  ...
  c3_w_tmp wyt_w = bitset_wyt(&bit_u);
  if ( 3 != wyt_w ) {
