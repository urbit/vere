...
*/
static u3_hbod*
_cttp_bod_new(c3_w_new len_w, c3_c* hun_c)
{
...
{
  c3_w_new len_w     = hed_u->nam_w + 2 + hed_u->val_w + 2;
  u3_hbod* bod_u = c3_malloc(1 + len_w + sizeof(*bod_u));
  bod_u->hun_y[len_w] = 0;
...
*/
static h2o_iovec_t*
_cttp_bods_to_vec(u3_hbod* bod_u, c3_w_new* tot_w)
{
  h2o_iovec_t* vec_u;
  c3_w_new len_w;
...
{
  c3_w_new     nam_w = u3r_met(3, nam);  //
  c3_w_new     val_w = u3r_met(3, val);  //  XX: potential truncations
...
*/
static void
_cttp_cres_new(u3_creq* ceq_u, c3_w_new sas_w)
{
...
*/
static c3_w_new
_cttp_mcut_pork(c3_c* buf_c, c3_w_new len_w, u3_noun pok)
{
...
*/
static c3_w_new
_cttp_mcut_quay(c3_c* buf_c, c3_w_new len_w, u3_noun quy)
{
  u3_noun yuq = quy;
  c3_o  fir_o = c3y;
...
*/
static c3_w_new
_cttp_mcut_url(c3_c* buf_c, c3_w_new len_w, u3_noun pul)
{
...
{
  c3_w_new  len_w = _cttp_mcut_url(0, 0, u3k(pul));
...
*/
static u3_creq*
_cttp_creq_find(u3_cttp* ctp_u, c3_l_new num_l)
{
 ...
 */
static u3_creq*
_cttp_creq_new(u3_cttp* ctp_u, c3_l_new num_l, u3_noun hes)
{
...
{
  {
    c3_w_new  len_w = strlen(ceq_u->met_c) + 1 + strlen(ceq_u->url_c) + 12;
...
static void
_cttp_http_client_receive(u3_creq* ceq_u, c3_w_new sas_w, u3_noun mes, u3_noun uct)
{
  u3_cttp* ctp_u = ceq_u->ctp_u;
                   ...
                   size_t hed_t, c3_i len_i)
{
  ...
  _cttp_cres_new(ceq_u, (c3_w_new)sas_i);
...
_cttp_ef_http_client(u3_cttp* ctp_u, u3_noun tag, u3_noun dat)
{
  ...
  if ( c3y == u3r_sing_c("request", tag) ) {
    ...
    if (  (c3n == u3r_cell(dat, &num, &req))
       || (c3n == u3r_safe_word_new(num, &num_l)) )
    {
    ...
    if ( c3n == u3r_safe_word_new(dat, &num_l) ) {
