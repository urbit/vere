...
typedef struct {
  ...
  /// Back pointer to enclosing client.
  struct client* client_u;
} _stream;
...
typedef struct client {
  ...
  struct {
    ...
    /// Input stream to IO process.
    _stream input_u;

    /// Output stream from IO process.
    _stream output_u;
  } child_u;
} _client;
...
_driver_kick(u3_auto* driver_u, u3_noun wire, u3_noun card)
{
  ...
  _client*    client_u = (_client*)driver_u;
  io_request* io_req_u = c3_malloc(sizeof(*io_req_u));
  u3s_jam_xeno(card, &io_req_u->len_d, &io_req_u->req_y);
  ...
  try_uv(uv_write(write_req_u,
                  (uv_stream_t*)&client_u->child_u.input_u,
                  req_bufs_u,
...
{
  static c3_d         idx_d  = 0;
  static io_response* resp_u = NULL;
  if ( !resp_u ) {
  ...
  if ( idx_d < sizeof(resp_u->len_d) ) {
    ssize_t len_bytes_i = c3_min(bytes_read_i, sizeof(resp_u->len_d) - idx_d);
  ...
  if ( bytes_read_i > 0 ) {
    ...
    ssize_t resp_bytes_i   = c3_min(bytes_read_i, bytes_needed_d);
    ...
    if ( idx_d == sizeof(resp_u->len_d) + resp_u->len_d ) {
      ...
      if ( u3_none != resp
           ...
           && c3y == u3a_is_cat(req_num) && c3y == u3a_is_cat(status) )
      {
        u3_noun wire = u3nt(u3i_string("http-client"),
                            u3dc("scot", c3__uv, client_u->inst_num_l),
                            u3_nul);
        u3_noun card
          = u3nt(u3i_string("receive"),
                 req_num,
                 u3nq(u3i_string("start"), u3nc(status, headers), body, c3y));
        u3m_p("wire", wire);
        u3m_p("card", card);
...
u3_cttp_io_init(u3_pier* const pir_u)
{
...

  {
    ...
    _stream* input_u = &client_u->child_u.input_u;
    try_uv(uv_pipe_init(u3L, &input_u->pipe_u, 0),
           goto fail,
           "failed to open pipe to stdin");
    input_u->client_u = client_u;

    _stream* output_u = &client_u->child_u.output_u;
    try_uv(uv_pipe_init(u3L, &output_u->pipe_u, 0),
           goto fail,
           "failed to open pipe to stdout");
    output_u->client_u = client_u;

    uv_stdio_container_t stdio_u[] = {
      [STDIN_FILENO] = {
        .flags       = UV_CREATE_PIPE | UV_READABLE_PIPE,
        .data.stream = (uv_stream_t*)&input_u->pipe_u,
      },
      [STDOUT_FILENO] = {
        .flags       = UV_CREATE_PIPE | UV_WRITABLE_PIPE,
        .data.stream = (uv_stream_t*)&output_u->pipe_u,
      },
      [STDERR_FILENO] = {
        .flags       = UV_INHERIT_FD,
    ...
    try_uv(
      uv_read_start((uv_stream_t*)&output_u->pipe_u, _read_alloc_cb, _read_cb),
      goto fail,
