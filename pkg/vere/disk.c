...
_disk_commit_cb(uv_work_t* ted_u)
{
  ...
  log_u->sav_u.ret_o = u3_book_save(log_u->txt_u,
                                    log_u->sav_u.eve_d,
                                    log_u->sav_u.len_d,
...
u3_disk_sync(u3_disk* log_u)
{
  ...
  //
  if ( c3y == _disk_batch(log_u) ) {
    ret_o = u3_book_save(log_u->txt_u,
                         log_u->sav_u.eve_d,
                         log_u->sav_u.len_d,
                 (void**)log_u->sav_u.byt_y,
                         log_u->sav_u.siz_i,
...
u3_disk_read_list(u3_disk* log_u, c3_d eve_d, c3_d len_d, c3_h* mug_h)
{
  ...
  if ( c3n == u3_book_read(log_u->txt_u, &ven_u,
                  ...
                  c3_d     len_d)
{
  ...
  wok_u->log_u = log_u;
  wok_u->liv_o = u3_book_walk_init(log_u->txt_u,
                                  &wok_u->itr_u,
                                   eve_d,
...
u3_disk_save_meta(MDB_env* mdb_u, const u3_meta* met_u)
{
  ...
  if (  (c3n == u3_lmdb_save_meta(mdb_u, "version", sizeof(c3_h), (c3_y*)&met_u->ver_h))
     ...
     || (c3n == u3_lmdb_save_meta(mdb_u, "life", sizeof(c3_h), (c3_y*)&met_u->lif_h)) )
  {
    u3z(who);
    return c3n;
  }
...
_disk_epoc_roll(u3_disk* log_u, c3_d epo_d)
{
  ...
  if ( c3n == u3_book_save_meta(log_u->txt_u, "version", sizeof(c3_h), (c3_y*)&old_u.ver_h)
     ...
     || c3n == u3_book_save_meta(log_u->txt_u, "life", sizeof(c3_h), (c3_y*)&old_u.lif_h) ) {
...
_disk_migrate_epoc(u3_disk* log_u, c3_d eve_d)
{
  ...
  if ( 0 == (log_u->txt_u = u3_book_init(tmp_c)) ) {
  ...
  if ( c3n == u3_book_save_meta(log_u->txt_u, "version", sizeof(c3_h), (c3_y*)&olm_u.ver_h)
     ...
     || c3n == u3_book_save_meta(log_u->txt_u, "life", sizeof(c3_h), (c3_y*)&olm_u.lif_h) ) {
...
_disk_migrate_old(u3_disk* log_u)
{
  ...
  switch ( log_u->ver_h ) {
    case U3D_VER1: {
      ...
      if ( c3n == u3_lmdb_save_meta(log_u->mdb_u, "version", sizeof(c3_h), (c3_y*)&log_u->ver_h) ) {
