...
_test_lily()
{
  ...
  u3_noun cod = u3dc("scot", c3_tas(uv), big);

  if ( c3y == u3v_lily(c3_tas(uv), cod, &lit_l) ) {
    printf("*** fail _test_lily-1\n");
    exit(1);
  }
  cod = u3dc("scot", c3_tas(ud), 0x7fffffff);
  if ( (c3n == u3v_lily(c3_tas(ud), cod, &lit_l)) ||
       (0x7fffffff != lit_l) ) {
    printf("*** fail _test_lily-2a\n");
    exit(1);
  }
  cod = u3dc("scot", c3_tas(ux), u3i_word(0x80000000));
  if ( c3y == u3v_lily(c3_tas(ux), cod, &lit_l) ) {
