...
u3_lmdb_init(const c3_c* pax_c, size_t siz_i)
{
  ...
  if ( (ret_h = mdb_env_create(&env_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: init fail");
    return 0;
  }
  ...
  if ( (ret_h = mdb_env_set_mapsize(env_u, siz_i)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: failed to set database size");
    //  XX dispose env_u
    //
    return 0;
  }
...
#   if defined(U3_OS_no_ubc)
      c3_h ops_h = MDB_WRITEMAP;
#   else
      c3_h ops_h = 0;
#   endif

    if ( (ret_h = mdb_env_open(env_u, pax_c, ops_h, 0664)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: failed to open event log");
      //  XX dispose env_u
      //
      return 0;
    }
...
u3_lmdb_gulf(MDB_env* env_u, c3_d* low_d, c3_d* hig_d)
{
  ...
  if ( (ret_h = mdb_txn_begin(env_u, 0, 0, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: gulf: txn_begin fail");
    return c3n;
  }
  ...
  //
  {
    ...
    if ( (ret_h = mdb_dbi_open(txn_u, "EVENTS", ops_h, &mdb_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: gulf: dbi_open fail");
      //  XX confirm
      //
      mdb_txn_abort(txn_u);
      return c3n;
...

  {
    ...
    if ( (ret_h = mdb_cursor_open(txn_u, mdb_u, &cur_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: gulf: cursor_open fail");
      //  XX confirm
      //
      mdb_txn_abort(txn_u);
      return c3n;
    ...
    if ( MDB_NOTFOUND == ret_h ) {
      ...
      *hig_d = 0;
      ...
      mdb_logerror(stderr, ret_h, "lmdb: gulf: head fail");
      mdb_cursor_close(cur_u);
      mdb_txn_abort(txn_u);
      return c3n;
    }
    ...
    //
    ret_h = mdb_cursor_get(cur_u, &key_u, &val_u, MDB_LAST);
    ...
    if ( ret_h ) {
      mdb_logerror(stderr, ret_h, "lmdb: gulf: last fail");
      return c3n;
    }
    else {
      *low_d = fir_d;
             ...
             c3_o   (*read_f)(void*, c3_d, size_t, void*))
{
  ...
  if ( (ret_h = mdb_txn_begin(env_u, 0, MDB_RDONLY, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: read txn_begin fail");
    return c3n;
  }
  ...
  //
  {
    ...
    if ( (ret_h = mdb_dbi_open(txn_u, "EVENTS", ops_h, &mdb_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: read: dbi_open fail");
      //  XX confirm
      //
      mdb_txn_abort(txn_u);
      return c3n;
...

  {
    ...
    //
    if ( (ret_h = mdb_cursor_open(txn_u, mdb_u, &cur_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: read: cursor_open fail");
      //  XX confirm
      //
      mdb_txn_abort(txn_u);
      return c3n;
    ...
    //
    if ( (ret_h = mdb_cursor_get(cur_u, &key_u, &val_u, MDB_SET_KEY)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: read: initial cursor_get failed at %" PRIu64, eve_d);
      mdb_cursor_close(cur_u);
      //  XX confirm
      //
      mdb_txn_abort(txn_u);
    ...
    //
    {
      ...
      for ( i_d = 0; (ret_h != MDB_NOTFOUND) && (i_d < len_d); ++i_d) {
        ...
        //
        if (  (ret_h = mdb_cursor_get(cur_u, &key_u, &val_u, MDB_NEXT))
           && (MDB_NOTFOUND != ret_h) )
        {
          mdb_logerror(stderr, ret_h, "lmdb: read: error");
          ret_o = c3n;
          break;
        }
      }
             ...
             size_t*  siz_i)               //  array of lengths
{
  ...
  //
  if ( (ret_h = mdb_txn_begin(env_u, 0, 0, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: write: txn_begin fail");
    return c3n;
  }
  ...
  //
  {
    ...
    if ( (ret_h = mdb_dbi_open(txn_u, "EVENTS", ops_h, &mdb_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: write: dbi_open fail");
      mdb_txn_abort(txn_u);
      return c3n;
    }
  }
  ...
  //
  {
    ...
    for ( i_d = 0; i_d < len_d; ++i_d) {
...

      {
        ...
        if ( (ret_h = mdb_put(txn_u, mdb_u, &key_u, &val_u, ops_h)) ) {
  ...
  if ( (ret_h = mdb_txn_commit(txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: write failed");
    return c3n;
  }
                  ...
                  void (*read_f)(void*, ssize_t, void*))
{
  ...
  //
  if ( (ret_h = mdb_txn_begin(env_u, 0, MDB_RDONLY, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: meta read: txn_begin fail");
    read_f(ptr_v, -1, 0);
    return;
  }
  ...
  //  open the database in the transaction
  //
  if ( (ret_h =  mdb_dbi_open(txn_u, "META", 0, &mdb_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: meta read: dbi_open fail");
    mdb_txn_abort(txn_u);
    read_f(ptr_v, -1, 0);
    return;
  }
  ...
  //  read by string key, invoking callback with result
  {
    ...
    if ( (ret_h = mdb_get(txn_u, mdb_u, &key_u, &val_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: read failed");
      mdb_txn_abort(txn_u);
      read_f(ptr_v, -1, 0);
      return;
    }
                  ...
                  void*       val_p)
{
  ...
  //
  if ( (ret_h = mdb_txn_begin(env_u, 0, 0, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: meta write: txn_begin fail");
    return c3n;
  }

  //  opens the database in the transaction
  //
  if ( (ret_h = mdb_dbi_open(txn_u, "META", MDB_CREATE, &mdb_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: meta write: dbi_open fail");
    mdb_txn_abort(txn_u);
    return c3n;
  }
  ...
  //
  if ( (ret_h = mdb_txn_commit(txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: meta write: commit failed");
    return c3n;
  }
                  ...
                  c3_d          las_d)
{
  ...
  //
  ops_h = MDB_RDONLY;
  if ( (ret_h = mdb_txn_begin(env_u, 0, ops_h, &itr_u->txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: read txn_begin fail");
    return c3n;
  }
  //  open the database in the transaction
  //
  ops_h = MDB_CREATE | MDB_INTEGERKEY;
  if ( (ret_h = mdb_dbi_open(itr_u->txn_u, "EVENTS", ops_h, &itr_u->mdb_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: read: dbi_open fail");
    //  XX confirm
    //
  ...
  //  creates a cursor to iterate over keys starting at [eve_d]
  //
  if ( (ret_h = mdb_cursor_open(itr_u->txn_u, itr_u->mdb_u, &itr_u->cur_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: read: cursor_open fail");
    //  XX confirm
    //
    mdb_txn_abort(itr_u->txn_u);
  ...
  //
  ops_h = MDB_SET_KEY;
  if ( (ret_h = mdb_cursor_get(itr_u->cur_u, &key_u, &val_u, ops_h)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: read: initial cursor_get failed");
...
u3_lmdb_walk_next(u3_lmdb_walk* itr_u, size_t* len_i, void** buf_v)
{
  ...
  ops_h = ( c3y == itr_u->red_o ) ? MDB_NEXT : MDB_GET_CURRENT;
  if ( (ret_h = mdb_cursor_get(itr_u->cur_u, &key_u, &val_u, ops_h)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: walk error");
    return c3n;
  }
...
**   caller must c3_free(*out_d).  returns c3n if no entry exists.
*/
c3_o
                   ...
                   c3_z*    out_z)
{
  ...
  if ( (ret_h = mdb_txn_begin(env_u, 0, MDB_RDONLY, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: blobs read: txn_begin fail");
    return c3n;
  }

  //  open the BLOBS database (read-only; may not exist yet)
  //
  {
    c3_h ops_h = MDB_INTEGERKEY;
  ...
  //
  {
    ...
    *out_z = val_u.mv_size / sizeof(c3_d);
    *out_d = c3_malloc(val_u.mv_size);
    memcpy(*out_d, val_u.mv_data, val_u.mv_size);
  }
...
/* u3_lmdb_walk_blobs(): iterate BLOBS table for events in [lo_d, hi_d].
**
**   calls [fun_f] for each event that has blob refs.
*/
void
                   ...
                   void  (*fun_f)(void*, c3_d, c3_d*, c3_z))
{
  ...
  if ( (ret_h = mdb_txn_begin(env_u, 0, MDB_RDONLY, &txn_u)) ) {
    mdb_logerror(stderr, ret_h, "lmdb: blobs walk: txn_begin fail");
    return;
  }

  {
    ...
    if ( (ret_h = mdb_dbi_open(txn_u, "BLOBS", ops_h, &mdb_u)) ) {
      //  database doesn't exist yet -- nothing to walk
      //
      mdb_txn_abort(txn_u);
      return;
    }
  ...
  {
    MDB_cursor* cur_u;
    MDB_val     key_u = { .mv_size = sizeof(c3_d), .mv_data = &lo_d };
    MDB_val     val_u;

    if ( (ret_h = mdb_cursor_open(txn_u, mdb_u, &cur_u)) ) {
      mdb_logerror(stderr, ret_h, "lmdb: blobs walk: cursor_open fail");
      mdb_txn_abort(txn_u);
      return;
    }

    //  position at first key >= lo_d
    //
    ret_h = mdb_cursor_get(cur_u, &key_u, &val_u, MDB_SET_RANGE);

    while ( !ret_h ) {
      c3_d eve_d = c3_sift_chub(key_u.mv_data);
