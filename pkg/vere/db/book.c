...
*/
static c3_h
_book_crc32(c3_y* buf_y, c3_h len_w)
{
  return (c3_h)crc32(0L, buf_y, len_w);
}

/* _book_crc32_two(): compute CRC32 over two buffers.
*/
static c3_h
_book_crc32_two(c3_y* one_y, c3_h one_w, c3_y* two_y, c3_h two_w)
{
  c3_h crc_w = (c3_h)crc32(0L, one_y, one_w);
  return (c3_h)crc32(crc_w, two_y, two_w);
}
                  ...
                  c3_d*  let_d)
{
  ...
  *jam_y = c3_malloc(jam_len);
  ...
  //  read crc_m (4 bytes)
  ret_zs = pread(fid_i, crc_w, sizeof(c3_h), off_now);
  if ( ret_zs != sizeof(c3_h) ) {
    c3_free(*mug_y);
    c3_free(*jam_y);
    return c3n;
  }
...
{
  c3_h off_w = sizeof(u3_book_head) + BOOK_META_SIZE;  //  events start here
  c3_d count_d = 0;
  c3_d expected_d;
  ...
  while ( 1 ) {
    ...
    //  validate CRC: CRC32(len_d || mug || jam)
    {
      ...
      calc_crc = (c3_h)crc32(calc_crc, jam_y, len_d - 4);
    }
             ...
             c3_o  (*read_f)(void*, c3_d, c3_z, void*))
{
  ...
  for ( i = 0; i < len_d; i++, cur_d++ ) {
    ...
    //  validate CRC
    {
      ...
      calc_crc = (c3_h)crc32(calc_crc, jam_y, len_rec - 4);
    }
                  ...
                  void*       val_p)
{
  ...
  if ( old_meta ) {
    ...
    for ( i = 0; i < old_count; i++ ) {
      c3_h entry_key_len, entry_val_len;
  ...
  offset += key_len;
  {
    c3_h val_len_w = (c3_h)val_z;  //  convert c3_z to c3_h for 4-byte field
  ...
  c3_h new_off = sizeof(u3_book_head);
