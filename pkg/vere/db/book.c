...
{
  c3_z len_z = offsetof(u3_book_head, crc_h);
...
{
  if ( BOOK_MAGIC != hed_u->mag_h ) {
    return c3n;
  }

  if ( BOOK_VERSION != hed_u->ver_h ) {
    return c3n;
  }

  c3_h crc_h = _book_head_crc(hed_u);
  if ( crc_h != hed_u->crc_h ) {
    return c3n;
  }
...
/* _book_save_head(): write header to inactive slot, sync, swap active.
**
**   caller must set hed_u fields (e.g. las_d) before calling.
**   increments seq_d, recomputes crc_h, and swaps active slot.
*/
static c3_o
_book_save_head(u3_book* txt_u)
{
  txt_u->hed_u.seq_d++;
  txt_u->hed_u.crc_h = _book_head_crc(&txt_u->hed_u);

  c3_d slot_d = (txt_u->act_h == 0) ? BOOK_HEAD_B : BOOK_HEAD_A;
...
**
**    caller should set fir_d and las_d on txt_u->hed_u before calling
**    (e.g. to epoch base for non-zero epochs, or 0 for fresh logs).
...
{
  c3_zs ret_zs;
  c3_d  fir_d = txt_u->hed_u.fir_d;
  c3_d  las_d = txt_u->hed_u.las_d;

  memset(&txt_u->hed_u, 0, sizeof(u3_book_head));
  txt_u->hed_u.mag_h = BOOK_MAGIC;
  txt_u->hed_u.ver_h = BOOK_VERSION;
  txt_u->hed_u.fir_d = fir_d;
  ...
  txt_u->act_h = 0;  //  start with slot A as active
...
**
**   on success, txt_u->hed_u contains the valid header and txt_u->act_h
...
_book_scan_back(u3_book* txt_u, c3_d* off_d)
{
  ...
  //  validate last deed by reading its trailing length field
  ...
  c3_d siz_d = _book_deed_size(let_d);
...

  {
    ...
    if ( txt_u->hed_u.bat_h > 0 ) {
      //  walk backward through bat_h deeds to find batch start
      c3_d  cur_d = pos_d;

      for ( c3_d i_d = 0; i_d < txt_u->hed_u.bat_h; i_d++ ) {
      ...
      c3_h sum_h = (c3_h)crc32(0, bat_y, byt_d);
      c3_free(bat_y);

      if ( sum_h != txt_u->hed_u.sum_h ) {
...
u3_book_init(const c3_c* pax_c)
{
  ...
  if ( buf_u.st_size == 0 ) {
    ...
    //  set epoch fields before writing header
             ...
             c3_d     epo_d)               //  target epoch
{
  ...
  c3_d dun_d = 0;
  c3_h chk_h = (c3_h)crc32(0, Z_NULL, 0);
  ...
  while ( dun_d < len_d ) {
    ...
    for ( c3_d i_d = 0; i_d < cun_d; i_d++ ) {
      ...
      chk_h = (c3_h)crc32(chk_h, (const c3_y*)&siz_i[src_d], sizeof(c3_d));
      chk_h = (c3_h)crc32(chk_h, buf_y, siz_i[src_d]);
      chk_h = (c3_h)crc32(chk_h, (const c3_y*)&siz_i[src_d], sizeof(c3_d));
  ...
  txt_u->hed_u.sum_h = chk_h;
  txt_u->hed_u.bat_h = len_d;
                  ...
                  void     (*read_f)(void*, c3_zs, void*))
{
  ...
  if ( 0 == strcmp(key_c, "version") ) {
    read_f(ptr_v, sizeof(c3_h), &met_u.ver_h);
  }
    ...
    read_f(ptr_v, sizeof(c3_h), &met_u.lif_h);
  }
  else {
    read_f(ptr_v, -1, 0);
  }
                  ...
                  void*       val_p)
{
  ...
  if ( 0 == strcmp(key_c, "version") ) {
    if ( val_z != sizeof(c3_h) ) return c3n;
    memcpy(&met_u.ver_h, val_p, val_z);
  }
  else if ( 0 == strcmp(key_c, "who") ) {
    ...
    if ( val_z != sizeof(c3_h) ) return c3n;
    memcpy(&met_u.lif_h, val_p, val_z);
  }
  else {
    return c3n;
  }
