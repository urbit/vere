...
/* _ce_image_stat_64(): measure 64-bit image.
*/
static _ce_img_stat_64
_ce_image_stat_64(u3e_image* img_u, c3_w* pgs_w)
{
  struct stat buf_u;
  ...
  if ( -1 == fstat(img_u->fid_i, &buf_u) ) {
    ...
    u3_assert(0);
    return _ce_img_fail_64;
  }
  else {
    c3_z siz_z = buf_u.st_size;
    c3_z pgs_z = (siz_z + (_ce_page_64 - 1))
                   >> (u3a_64_page + u3a_64_word_bytes_shift);

    if ( !siz_z ) {
      *pgs_w = 0;
      return _ce_img_good_64;
    }
    else if ( siz_z != _ce_len_64(pgs_z) ) {
      fprintf(stderr, "loom: image corrupt size %zu\r\n", siz_z);
      return _ce_img_size_64;
    }
...
/* _ce_image_open_64(): open 64-bit image file.
*/
static _ce_img_stat_64
_ce_image_open_64(u3e_image* img_u, c3_c* ful_c)
{
  c3_i mod_i = O_RDONLY;
  ...
  return _ce_image_stat_64(img_u, &img_u->pgs_w);
}
...
u3e_64_image_open_any(c3_c* nam_c, c3_c* dir_c, c3_z* len_z)
{
  ...
  switch ( _ce_image_open_64(&img_u, dir_c) ) {
    case _ce_img_good_64: {
      *len_z = _ce_len_64(img_u.pgs_w);
      return img_u.fid_i;
    } break;

    case _ce_img_fail_64:
    case _ce_img_size_64: {
      *len_z = 0;
      return -1;
    } break;
  }
