...
static c3_d
_v5_64_hash(u3_v5_64_noun foo)
{
  return foo * 11400714819323198485ULL;
}

static c3_i
_v5_64_cmp(u3_v5_64_noun a, u3_v5_64_noun b)
{
  return a == b;
}

#define NAME    _v5_64_to_native
#define KEY_TY  u3_v5_64_noun
#define VAL_TY  u3_noun
#define HASH_FN _v5_64_hash
#define CMPR_FN _v5_64_cmp
#include "verstable.h"

typedef struct {
  u3_weak       hed;  // native noun head (u3_none if not yet copied)
  u3_v5_64_noun cel;  // v5 64-bit cell being processed
} _copy_frame;

typedef struct {
  _v5_64_to_native map_u;
  c3_w             len_w;
  c3_w             siz_w;
  _copy_frame     *tac;
  u3p(u3h_root)    ham_p;
...
/* _copy_atom(): copy a v5 64-bit indirect atom into the native 32-bit loom.
**
**   v5 64-bit atoms: len_w counts uint64_t words in buf_w
**   native 32-bit atoms: len_w counts uint32_t words in buf_w
**   conversion: new_len_w = old_len_w * 2, then trim trailing zero uint32_t words
**
**   normalization: native 32-bit cats are 31-bit (≤ 2^31-1); v5 64-bit atoms
**   with such small values must become native cats, not indirect atoms
*/
static u3_atom
_copy_atom(u3_v5_64_noun old)
{
  u3a_v5_64_atom *old_u = (u3a_v5_64_atom *)u3a_v5_64_to_ptr(old);
  c3_v5_64_w      old_w = old_u->len_w;  /* count of uint64_t words */
  ...
  /* normalize to native cat if value fits in 31 bits */
...
/* _copy_v5_64_next(): iterative descent into a v5 64-bit noun, returning the
**   native 32-bit copy.
**
**   v5 64-bit cats with values ≥ 2^31 cannot be native 32-bit cats; they
**   become native indirect atoms
*/
static u3_noun
_copy_v5_64_next(_copy_ctx *cop_u, u3_v5_64_noun old)
{
  _v5_64_to_native_itr vit_u;
  _copy_frame         *top_u;

  while ( 1 ) {
    if ( c3y == u3a_v5_64_is_cat(old) ) {
      //  direct atom in v5 64-bit
      if ( old <= u3a_32_direct_max ) {
        return (u3_noun)old;  //  fits in native 32-bit cat
      }
      else {
        //  v5 64-bit cat ≥ 2^31: must become a native indirect atom
    ...
    top_u->hed = u3_none;
    top_u->cel = old;
    old = u3a_v5_64_head(old);
    continue;
  }
}

static u3_noun
_copy_v5_64_noun(_copy_ctx *cop_u, u3_v5_64_noun old)
{
  _v5_64_to_native_itr vit_u;
  _copy_frame         *top_u;
  u3_noun               new;
  ...
  new = _copy_v5_64_next(cop_u, old);
  ...
  while ( cop_u->len_w ) {
    ...
    if ( u3_none == top_u->hed ) {
      top_u->hed = new;
      new = _copy_v5_64_next(cop_u, u3a_v5_64_tail(top_u->cel));
    }
    else {
...
static void
_copy_v5_64_hamt(u3_v5_64_noun kev, void* ptr_v)
{
  _copy_ctx *cop_u = ptr_v;
  u3_noun key = _copy_v5_64_noun(cop_u, u3a_v5_64_head(kev));
  u3_noun val = _copy_v5_64_noun(cop_u, u3a_v5_64_tail(kev));
...
u3_migrate_32(c3_d eve_d)
{
  ...
  u3A->eve_d = u3A_v5_64->eve_d;
  u3A->roc   = _copy_v5_64_noun(&cop_u, u3A_v5_64->roc);

  cop_u.ham_p = u3R->jed.cod_p;
  u3h_v5_64_walk_with(u3R_v5_64->jed.cod_p, _copy_v5_64_hamt, &cop_u);
  cop_u.ham_p = u3R->cax.per_p;
  u3h_v5_64_walk_with(u3R_v5_64->cax.per_p, _copy_v5_64_hamt, &cop_u);
  cop_u.ham_p = u3R->cax.for_p;
  u3h_v5_64_walk_with(u3R_v5_64->cax.for_p, _copy_v5_64_hamt, &cop_u);
